...
{ 
   ...
   MACNET_rx_free_specific(macnet_context_ptr, pcb_ptr, (void **)&macnet_context_ptr->SMALL_BUFFERS);
}
...
{ 
   ...
   MACNET_rx_free_specific(macnet_context_ptr, pcb_ptr, (void **)&macnet_context_ptr->LARGE_BUFFERS);
}
   ...
   */
   MACNET_int_disable();
      ...
      for (i=0;pcb_ptr->FRAG[i].FRAGMENT;i++) {
         ENET_Enqueue_Buffer((void **)&macnet_context_ptr->RX_BUFFERS, pcb_ptr->FRAG[i].FRAGMENT );
         pcb_ptr->FRAG[i].FRAGMENT = NULL;
      }
   #else   
      ENET_Enqueue_Buffer((void **)&macnet_context_ptr->RX_BUFFERS, pcb_ptr->FRAG[0].FRAGMENT );
      pcb_ptr->FRAG[0].FRAGMENT = NULL;
   #endif
...
{ 
   ...
   */
   ...
   while (macnet_context_ptr->ActiveRxBDs < macnet_context_ptr->NumRxBDs) {
      ...
      buf = (unsigned char *)HOST_TO_BE_LONG((uint32_t)ENET_Dequeue_Buffer((void **) &macnet_context_ptr->RX_BUFFERS));
      if (buf == NULL) {
         MACNET_int_enable();
         return;
      }
      ...
      */
      ...
      _DCACHE_FLUSH_MBYTES((void *)bd_ptr, sizeof(ENET_BD_STRUCT));  
...
static void MACNET_discard_current_rx(ENET_CONTEXT_STRUCT_PTR    enet_ptr,MACNET_CONTEXT_STRUCT_PTR   macnet_context_ptr, bool last)
{
   uint32_t  i;
   ...
   if (macnet_context_ptr->CurrentRxECB) {
      for (i=0;i<macnet_context_ptr->CurrentRxFrag;i++) {
         ...
         if (macnet_context_ptr->FRAGS[i].FRAGMENT) {
            ENET_Enqueue_Buffer((void **)&macnet_context_ptr->RX_BUFFERS, macnet_context_ptr->FRAGS[i].FRAGMENT);
         }
...
{ /* Body */
   ...
   uint32_t                    i,length;
   int32_t                     ilength;
   unsigned char                  *buffer, *small_packet, *large_packet;
   bool                    buffer_is_valid, consumed, first, last;
   ...
   while (macnet_context_ptr->ActiveRxBDs > 0) {
      ...
      buffer = (unsigned char *)LONG_BE_TO_HOST((uint32_t)(bd_ptr->BUFFER));
      ...
      if (rx_status & ENET_BD_ETHER_RX_TRUNCATED) {
         ...
         if (rx_status & ENET_BD_ETHER_RX_OVERRUN) {
            ...
            ENET_INC_STATS(ST_RX_FCS);
               ...
               uint32_t index = length>> ENET_HISTOGRAM_SHIFT;
         ...
         if (macnet_context_ptr->CurrentRxECB ) {
            ...
            if (!last) { 
               ...
               if (pcb_ptr) {
                  ...
                  pcb_ptr->PRIVATE = (void *)enet_ptr;
                  pcb_ptr->FREE = MACNET_rx_free;
                  
                  // Check to see if it is a large packet
                  if (!first) {
                     // Coalese
                     large_packet = ENET_Dequeue_Buffer((void **)&macnet_context_ptr->LARGE_BUFFERS);
                  ...
                  // Check to see if it is a small packet
                     ...
                     if (length < MACNET_SMALL_PACKET_SIZE) {
                        ...
                        small_packet = ENET_Dequeue_Buffer((void **)&macnet_context_ptr->SMALL_BUFFERS);
      ...
      if (!consumed) {
         ENET_INC_STATS(COMMON.ST_RX_DISCARDED);
         ENET_Enqueue_Buffer((void **)&macnet_context_ptr->RX_BUFFERS, buffer);
      }
   }
   MACNET_add_buffers_to_rx_ring(macnet_context_ptr);
} 
...
{ /* Body */
   ...
   uint16_t                    rx_status;
   uint32_t                    length,i;
   unsigned char                  *buffer, *small_packet;
   bool                    buffer_is_valid, processed, last;
