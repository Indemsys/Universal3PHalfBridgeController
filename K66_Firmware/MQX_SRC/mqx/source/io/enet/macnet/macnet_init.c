...
{ 
   ...
   */
   ...
   if(!MACNET_ptp_init(enet_ptr))
       IF_ERROR_EXIT( TRUE , ENETERR_1588TSBUFF_ALLOC);
   
   if (macnet_ptp_ptr->PTIMER_PRESENT) {
      /* Set Timer count */
	  MACNET_ptp_start(macnet_ptp_ptr, enet_ptr->PARAM_PTR->OPTIONS);
      if(enet_ptr->PARAM_PTR->OPTIONS & ENET_OPTION_PTP_MASTER_CLK){
           ...
           macnet_ptp_ptr->MACNET_PTR->EIMR |= ENET_EIR_TS_TIMER_MASK;
      }
