...
static _mqx_int _tchres_open( MQX_FILE_PTR, char *, char *);
static _mqx_int _tchres_close( MQX_FILE_PTR );
static _mqx_int _tchres_ioctl( MQX_FILE_PTR, _mqx_uint, void *);
static _mqx_int _tchres_get_position_raw( int16_t *, int16_t *, TCHRES_DEVICE_STRUCT_PTR );
static uint16_t _tchres_read_touch_axis( TCHRES_PIN_SELECT_ENUM, TCHRES_DEVICE_STRUCT_PTR );
static int16_t _tchres_get_raw_limits( TCHRES_LIMITS_ENUM, TCHRES_DEVICE_STRUCT_PTR );
static unsigned char _tchres_get_filtered_axis(TCHRES_DEVICE_STRUCT_PTR, TCHRES_PIN_SELECT_ENUM, uint16_t *);
static void _tchres_set_pins_to_default( TCHRES_DEVICE_STRUCT_PTR );
static _mqx_int _tchres_measure_position(TCHRES_PIN_SELECT_ENUM, int16_t *, TCHRES_DEVICE_STRUCT_PTR);
...
)
{
   ...
   if( tchscr_dev_ptr->COUNT == 0 )
   {
      ...
      ioctl(tchscr_dev_ptr->ADC_CHANNEL_X, ADC_IOCTL_PIN_DISABLE, (void *) tchscr_dev_ptr->ADC_CHANNEL_X_STRUCT->source);
      ioctl(tchscr_dev_ptr->ADC_CHANNEL_Y, ADC_IOCTL_PIN_DISABLE, (void *) tchscr_dev_ptr->ADC_CHANNEL_Y_STRUCT->source);
#else
...
*
*END*----------------------------------------------------------------------*/
static uint16_t _tchres_read_touch_axis
(
   TCHRES_PIN_SELECT_ENUM pin_select,
   TCHRES_DEVICE_STRUCT_PTR tchscr_dev_ptr
)
{
   ADC_RESULT_STRUCT data = {0};

   switch( pin_select )
   {
      case X_PLUS:
         ioctl(tchscr_dev_ptr->ADC_CHANNEL_X, ADC_IOCTL_FIRE_TRIGGER, (void *) tchscr_dev_ptr->ADC_CHANNEL_X_STRUCT->trigger);
      ...
      case Y_PLUS:
         ioctl(tchscr_dev_ptr->ADC_CHANNEL_Y, ADC_IOCTL_FIRE_TRIGGER, (void *) tchscr_dev_ptr->ADC_CHANNEL_Y_STRUCT->trigger);
...
*
*END*----------------------------------------------------------------------*/
static unsigned char _tchres_get_filtered_axis
(
        TCHRES_DEVICE_STRUCT_PTR tchscr_dev_ptr,
...
)
{
    ...
    uint16_t wSample[NBR_SAMPLES];
    uint16_t wADCReading, adcMin, adcMax, wRes = 0, wSum = 0;
    uint8_t tmp, tmp_res, deviation;

    TCHRES_ADC_LIMITS_STRUCT_PTR p_limits = &tchscr_dev_ptr->ADC_LIMITS;
    deviation = (unsigned char) (p_limits->FULL_SCALE / 256);
...
#if PSP_MQX_CPU_IS_MCF51JE || PSP_MQX_CPU_IS_MCF51MM
  ioctl(tchscr_dev_ptr->ADC_CHANNEL_X, ADC_IOCTL_PIN_DISABLE, (void *) 0);
  ioctl(tchscr_dev_ptr->ADC_CHANNEL_Y, ADC_IOCTL_PIN_DISABLE, (void *) 0);
#else
  lwgpio_set_functionality(&tchscr_dev_ptr->X_PLUS_PIN, tchscr_dev_ptr->PIN_FUNCT.X_PLUS_GPIO_FUNCTION);
  lwgpio_set_functionality(&tchscr_dev_ptr->Y_PLUS_PIN, tchscr_dev_ptr->PIN_FUNCT.Y_PLUS_GPIO_FUNCTION);
#endif
...
)
{
    ...
    uint16_t        tmpCnt = 0, tmpRes = 0, min_voltage = 0;
    uint32_t        adc_func;
    MQX_FILE_PTR    adc_channel = 0;
    (void)          adc_channel; /* suppress 'unused variable' warning */
    ...
    /* Switch to ADC */
 #if PSP_MQX_CPU_IS_MCF51JE || PSP_MQX_CPU_IS_MCF51MM
    ioctl(adc_channel, ADC_IOCTL_PIN_ENABLE, (void *) 0);
 #else
...
static _mqx_int _tchres_get_position_raw(
        int16_t *TouchPositionX,
        int16_t *TouchPositionY,
        TCHRES_DEVICE_STRUCT_PTR tchscr_dev_ptr)
{
    _mqx_int result;
...
*
*END*----------------------------------------------------------------------*/
static int16_t _tchres_get_raw_limits
(
