...
// TODO: unify error type cross all drivers !!!
    return ((_mqx_int)_io_dev_install_ext(identifier,
        _io_sdcard_open,
        _io_sdcard_close,
   ...
   )
{
    ...
    if ((fd_ptr->LOCATION + num) > sdcard_ptr->NUM_BLOCKS)
    {
        ...
        num = (_mqx_int)(sdcard_ptr->NUM_BLOCKS - fd_ptr->LOCATION);
    }

    _lwsem_wait(&sdcard_ptr->LWSEM);
    res = sdcard_ptr->INIT->READ_FUNC(fd_ptr, (unsigned char *)data_ptr, fd_ptr->LOCATION, (_mqx_uint)num);
   ...
   )
{
    ...
    _lwsem_wait(&sdcard_ptr->LWSEM);
    res = sdcard_ptr->INIT->WRITE_FUNC(fd_ptr, (unsigned char *)data_ptr, fd_ptr->LOCATION, (_mqx_uint)num);
...
)
{
    ...
    if (0 == (csd[0] & 0xC0))
    {
        ...
        c_size = (uint32_t)(csd[6] & 0x03);
        c_size = (uint32_t)((c_size << 8) | csd[7]);
        c_size = (uint32_t)((c_size << 2) | (csd[8] >> 6));
        c_size_mult = (uint32_t)(((csd[9] & 0x03) << 1) | (csd[10] >> 7));
        capacity = (uint32_t)((c_size + 1) * (1 << (c_size_mult + 2)) * (1 << (read_bl_len - 9)));
    }
    else
    {
        c_size = (uint32_t)(csd[7] & 0x3F);
        c_size = (uint32_t)((c_size << 8) | csd[8]);
        c_size = (uint32_t)((c_size << 8) | csd[9]);
        capacity = (uint32_t)((c_size + 1) << 10);
    }
...
)
{
    ...
    unit = (uint32_t)(csd[3] & 0x07);
    value = (uint32_t)((csd[3] & 0x78) >> 3);
