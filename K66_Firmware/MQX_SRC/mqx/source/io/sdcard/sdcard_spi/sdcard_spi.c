...
*
*END*----------------------------------------------------------------------*/
static uint32_t _io_sdcard_crc7
(
...
)
{
    ...
    while (len) {
        index = (int32_t)(((crc >> 3) & 0x0f) ^ ((*input) >> 4));
        crc = (crc << 4) ^ crc_table[index];

        index = (int32_t)(((crc >> 3) & 0x0f) ^ ((*input) & 0x0f));
        crc = (crc << 4) ^ crc_table[index];
...
static uint32_t _io_sdcard_crc16
(
    /* [IN] Initial value, maybe previous partial result */
    uint32_t crc,
...
)
{
    ...
    while (len > 0) {
        index = (int32_t)(((crc>>12) & 0x0f) ^ ((*input) >> 4));
        crc = (crc<<4) ^ crc_table[index];

        index = (int32_t)(((crc>>12) & 0x0f) ^ ((*input) & 0x0f));
        crc = (crc<<4) ^ crc_table[index];
...
*
*END*----------------------------------------------------------------------*/
static uint32_t _io_sdcard_spi_read
(
...
)
{
    ...
    do
    {
        count += fread(buffer+count, (_mqx_int)size, (_mqx_int)(number - count), spifd);
    } while (count < number);
    return count;
}
...
*
*END*----------------------------------------------------------------------*/
static uint32_t _io_sdcard_spi_write
(
...
)
{
    ...
    do
    {
        count += fwrite (buffer + count, (_mqx_int)size, (_mqx_int)(number - count), spifd);
    } while (count < number);
    return count;
}
...
*
*END*----------------------------------------------------------------------*/
static bool _io_sdcard_spi_wait
(
    /* [IN] SD card info */
...
*
*END*----------------------------------------------------------------------*/
static bool _io_sdcard_spi_command
(
    /* [IN] SD card info */
    SDCARD_STRUCT_PTR sdcard_ptr,
...
)
{
    ...
    buffer[0] = command;
    buffer[1] = (uint8_t)((param >> 24) & 0xFF);
    buffer[2] = (uint8_t)((param >> 16) & 0xFF);
    buffer[3] = (uint8_t)((param >>  8) & 0xFF);
    buffer[4] = (uint8_t)(param & 0xFF);
    buffer[5] = (uint8_t)((_io_sdcard_crc7(0, buffer, 5) << 1) | SDCARD_SPI_END_BIT);
...
*
*END*----------------------------------------------------------------------*/
static bool _io_sdcard_spi_read_data
(
    /* [IN] SD card info */
    SDCARD_STRUCT_PTR sdcard_ptr,
...
*
*END*----------------------------------------------------------------------*/
static bool _io_sdcard_spi_write_data
(
    /* [IN] SD card info */
    SDCARD_STRUCT_PTR sdcard_ptr,
...
)
{
    ...
    /* Calculate CRC */
    crc = (uint16_t)_io_sdcard_crc16 (0, data, data_len);
    tmp[0] = (uint8_t)(crc >> 8);
...
*
*END*----------------------------------------------------------------------*/
static bool _io_sdcard_spi_write_stop
(
    /* [IN] SD card info */
    SDCARD_STRUCT_PTR sdcard_ptr
)
...
)
{
    ...
    /* send CMD10 = get CID */
    if (!_io_sdcard_spi_command(sdcard_ptr, SDCARD_SPI_CMD10_SEND_CID, 0, &r1, NULL, 0))
        return FALSE;

    if (r1 & SDCARD_SPI_R1_ERROR_MASK)
        return FALSE;

    if (!_io_sdcard_spi_read_data(sdcard_ptr, sdcard_ptr->CID, 16))
        return FALSE;
