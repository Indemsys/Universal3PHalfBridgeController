...
(
      ...
      FLASHX_INIT_STRUCT const   *init_ptr
...
{ /* Body */
    ...
    char                *file_name_ptr = fd_ptr->DEV_PTR->IDENTIFIER; /* use driver name temporarily */
    bool                chip_initialized = TRUE;
...
{ /* Body */
    ...
    _mem_free((void *)file_ptr->ERASE_ARRAY);
...
{ /* Body */
    ...
    if (fd_ptr->LOCATION + num > fd_ptr->SIZE) {
        ...
        num = (_mqx_int)(fd_ptr->SIZE - fd_ptr->LOCATION);
    }
    ...
    while (remains)
    {
        ...
        if (copy_num > remains)
        copy_num = remains;
        _mem_copy((void *)(dev_ptr->BASE_ADDR + dev_ptr->HW_BLOCK[sb].START_ADDR + ss * dev_ptr->HW_BLOCK[sb].SECTOR_SIZE + offset), (void *) data_ptr, copy_num);
        remains -= copy_num;
        data_ptr += copy_num;
...
* Returned Value   : bool, TRUE upon success
...
{ /* Body */
    ...
    if (dev_ptr->SECTOR_ERASE) {
        ...
        if (0 == (file_ptr->ERASE_ARRAY[array_offset] & erase_mask)) {
            dest_ptr = (char *)(dev_ptr->BASE_ADDR + dev_ptr->HW_BLOCK[block_number].START_ADDR + sector_number * dev_ptr->HW_BLOCK[block_number].SECTOR_SIZE);
...
FLASHX_BLOCK_INFO_STRUCT_PTR _io_flashx_phys_to_sector
   (
      ...
      _mqx_uint       *sector_number_ptr,

      /* [IN/OUT] Physical address of the sector */
      _mem_size       *sector_addr_ptr,

      /* [IN/OUT] The offset within the sector */
      _mem_size       *sector_offset_ptr
...
* Returned Value   : _mqx_int, IO_OK upon SUCCESS
...
_mqx_int _io_flashx_find_correct_sectors
   (
      /* [IN] The device handle */
      MQX_FILE_PTR         fd_ptr,
    ...
    *start_block_ptr = 0;
    ...
    if (b->NUM_SECTORS != 0) {
        ...
        */
        ...
        /* Check if sector was find */
        if (b->NUM_SECTORS == 0) {
           /* The file is located after possible physical memory adress space,
           ** return error
           */
           return IO_ERROR;
        }
        ...
        *file_sector_ptr -= dummy_sect;
        ...
        /* the file is located after possible physical memory address space, return error*/
        return IO_ERROR;
    }
   ...
   ) 
{
    ...
    if (file_ptr->DIRTY_DATA) {
        sb = (_mqx_uint)file_ptr->CACHE_BLOCK;
        ss = (_mqx_uint)file_ptr->CACHE_SECTOR;
        fs = (_mqx_uint)file_ptr->CACHE_FILE_SECTOR;
        ...
        if ((*dev_ptr->SECTOR_PROGRAM)(dev_ptr, file_ptr->CACHE_PTR, (char *)dest_ptr, dev_ptr->HW_BLOCK[sb].SECTOR_SIZE))
        {
            error = (_mqx_int)dev_ptr->HW_BLOCK[sb].SECTOR_SIZE;
    ...
    while (remains) {
        ...
        if (offset || (remains < dev_ptr->HW_BLOCK[b].SECTOR_SIZE)) {
            ...
            */
            ...
            result = (_mqx_int)_io_flashx_write_partial_sector(fd_ptr, b, s, offset, copy_num, fs, data_ptr);
            if (result == IO_ERROR) {
            ...
            if (result != IO_ERROR) {
                ...
                if (!(*dev_ptr->SECTOR_PROGRAM)(dev_ptr, data_ptr, (char *) dest_ptr, copy_num))
                    result = IO_ERROR;
            }
            if (result == IO_ERROR) {
                fd_ptr->ERROR = IO_ERROR_WRITE;
...
{ /* Body */
    ...
    sector_dest_ptr = (char *)(dev_ptr->BASE_ADDR + dev_ptr->HW_BLOCK[block_number].START_ADDR + dev_ptr->HW_BLOCK[block_number].SECTOR_SIZE * sector_number);
    ...
    if (!file_ptr->DIRTY_DATA && !_io_flashx_check_free_space(data_dest_ptr, bytes_to_write) \
        ...
        /* Read the sector for partial write from flash into RAM only if the cache has not already been populated.
         * Otherwise, this operation will lost previous data in cache. */
        if (!file_ptr->DIRTY_DATA)
            _mem_copy((void *)sector_dest_ptr, file_ptr->CACHE_PTR, sector_size);
 
        file_ptr->CACHE_BLOCK  = (_mqx_int)block_number;
        file_ptr->CACHE_SECTOR = (_mqx_int)sector_number;
        file_ptr->CACHE_FILE_SECTOR = (_mqx_int)file_sector;

        /* Write new data into RAM cache sector */
        _mem_copy((void *)src_ptr, (void *)((char *) file_ptr->CACHE_PTR + offset_in_sector), bytes_to_write);
...
)
{
    ...
    if(num_bytes_to_check%8 != 0x0)
        num_bytes_to_check = ((num_bytes_to_check>>3)+1)<<3;
...
{ /* Body */
    ...
    switch (cmd) {
        ...
        case IO_IOCTL_GET_NUM_SECTORS:
            *(uint32_t *)param_ptr = fd_ptr->SIZE / MFS_SECTOR_SIZE;
            break;
        ...
        case IO_IOCTL_GET_BLOCK_SIZE:
            ...
            *(uint32_t *)param_ptr = MFS_SECTOR_SIZE;
            break;
        ...
        case FLASH_IOCTL_GET_BLOCK_MAP:
            ...
            *pparam_ptr = (void *) dev_ptr->HW_BLOCK;
            break;
        ...
        case FLASH_IOCTL_ERASE_SECTOR:
            ...
            */
            ...
            result = _io_flashx_find_correct_sectors(fd_ptr, fd_ptr->LOCATION, &sb, &ss, &offset, &fs);
            
            if( IO_OK != result)
                break;
            
            temp_ptr = (char *)(dev_ptr->BASE_ADDR + dev_ptr->HW_BLOCK[sb].START_ADDR + dev_ptr->HW_BLOCK[sb].SECTOR_SIZE * ss);
        ...
        case FLASH_IOCTL_ERASE_FILE:
            /* 
            ** erase all sectors belongs to the file 
            */
            if (!dev_ptr->SECTOR_ERASE) {
                result = IO_ERROR_INVALID_IOCTL_CMD;
                break;
            ...
            _lwsem_wait(&dev_ptr->HW_ACCESS);
            for (i = 0, j = (file_ptr->FILE_BLOCK->END_ADDR - file_ptr->FILE_BLOCK->START_ADDR); i <= j;) {
                _io_flashx_find_correct_sectors(fd_ptr, i, &sb, &ss, &offset, &fs);
                
                /* empty block, cause by incorrect address range of flashx file */
                if (!dev_ptr->HW_BLOCK[sb].NUM_SECTORS) {
                    result = IO_ERROR;
                    break;
                }
                
                /* erase sectors of current bank until cursor reach the end */
                for (; (i <= j) && (ss < dev_ptr->HW_BLOCK[sb].NUM_SECTORS); ss++) {
                    if ((*dev_ptr->SECTOR_ERASE)(dev_ptr, (char*)(file_ptr->FILE_BLOCK->START_ADDR + i), dev_ptr->HW_BLOCK[sb].SECTOR_SIZE)) {
                        file_ptr->ERASE_ARRAY[fs / 8] |= (0x1 << (fs % 8));
                        fs++;
                    }
                    else {
                    /* could not erase sector, end the loop */
                        result = IO_ERROR;
                        break;
                    }
                    i += dev_ptr->HW_BLOCK[sb].SECTOR_SIZE;
                }
                /* error occoured, end the loop */
                if (MQX_OK != result) {
                    break;
                }
            }
            ...
            ** associated with opened files on this driver.
            ...
            if (dev_ptr->CHIP_ERASE) {
                ...
                for (i = 0; dev_ptr->HW_BLOCK[i].NUM_SECTORS != 0; i++) {
                    temp_ptr = (char *)(dev_ptr->BASE_ADDR + dev_ptr->HW_BLOCK[i].START_ADDR);
...
{/* Body */
    ...
    start_us = (int32_t) _time_get_microseconds();
    while (total_us < wait_us) {
        end_us = (int32_t) _time_get_microseconds();
        if (end_us > start_us) {
