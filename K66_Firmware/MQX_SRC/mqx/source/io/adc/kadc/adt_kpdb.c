...
static volatile struct {
    ...
    void   *old_idelay_isr;
    void   *old_idelay_isr_data;
    _mqx_int old_isr_valid; /* which of interrupts handlers above are valid */
    uint32_t used_channel[ADC_MAX_MODULES];  /* mask of used channels */
    uint32_t delayA[ADC_MAX_MODULES]; /* if a channel A is paused, the original value of delay stored here */
} pdb_internal = { 0 };
...
#define ENABLE_CH0_PROCESSING(adc_num, adc_ch_bundle) \
    ...
    #define PDB_CH_EN(adc_num)\
        * (uint8_t *) (&PDB0_BASE_PTR->CH[adc_num].C1)
#else
    #define PDB_CH_EN(adc_num)\
        *(uint8_t *)((uint32_t)&PDB0_BASE_PTR->CH[adc_num].C1 + 3)
#endif
extern ADC_BUNDLE adc_bundle;

static _mqx_int adt_period_prescaler(uint32_t period);
static void adt_idelay_isr(void *param);
...
)
{
    ...
    switch (cmd)
    {
        case ADC_IOCTL_SET_IDELAY:
            ...
            d = (((uint64_t)((uint32_t)param_ptr)) * PDB0_BASE_PTR->MOD) / pdb_period;
            if (d > MAX_UINT_16)
        ...
        case ADC_IOCTL_SET_IDELAYREG:
            /* sets offset for interrupt in PDB module - see IDELAY register in Reference Manual */
            if ((uint32_t) param_ptr > MAX_UINT_16)
        ...
        case ADC_IOCTL_SET_BASE_PERIOD:
            ...
            if (pdb_period == 0) { /* period was not set yet? */
                return adt_period_prescaler((uint32_t) param_ptr);
            }
            else { /* trim period if possible */
                d = (uint32_t)(((((uint64_t)((uint32_t)param_ptr)) * PDB0_BASE_PTR->MOD) / pdb_period) - 1);
                if (d > MAX_UINT_16)
                    return ADC_ERROR_BAD_PARAM; /* cannot set such period */
                PDB0_BASE_PTR->MOD = (uint16_t) d;
            }
...
*
*END*********************************************************************/
_mqx_int _adt_hw_install(char *adc_name, ADC_DRIVER_BUNDLE_PTR adc_d_bundle, void *p)
{
    return ADC_OK;
}
...
{ /* Body */
    ...
    if (!(pdb_internal.old_isr_valid & 0x01)) {
        pdb_internal.old_idelay_isr_data = (void *) _int_get_isr_data(adc_d_bundle->init->PDB_INIT->IDELAY_VECTOR);
        pdb_internal.old_idelay_isr = (void *) _int_install_isr(adc_d_bundle->init->PDB_INIT->IDELAY_VECTOR, adt_idelay_isr, f);
    ...
    switch (adc_num)
    {
        ...
        case 1: 
            ...
            break;
#endif
#if defined(SIM_SOPT7_ADC2ALTTRGEN_MASK)
        case 2:
            sim_ptr->SOPT7 &= ~SIM_SOPT7_ADC2ALTTRGEN_MASK; /* set PDB to be source for ADC conversion start */
            break;
#endif
#if defined(SIM_SOPT7_ADC3ALTTRGEN_MASK)
        case 3: 
            sim_ptr->SOPT7 &= ~SIM_SOPT7_ADC3ALTTRGEN_MASK; /* set PDB to be source for ADC conversion start */
            break;
#endif
        default:
...
static _mqx_int adt_period_prescaler(uint32_t period)
{
    volatile uint32_t pdb_prescaler1, pdb_prescaler, pdb_period1, pdb_period = 0, pdb_period_test;
    uint8_t mult = 0; /* assume that we will not use MULT field */
    ...
    */
#define PDB_MAX_PERIOD1 (uint32_t) ((uint64_t) 8388608000000L / BSP_BUS_CLOCK)   /*  1 * 128 * 65536 * 1000000 = 8388608000000 */
#define PDB_MAX_PERIOD  (uint32_t) ((uint64_t) 335544320000000L / BSP_BUS_CLOCK) /* 40 * 128 * 65536 * 1000000 = 335544320000000 */
    ...
    if (pdb_external.prescaler == -1) {
        ...
        pdb_prescaler1 = 7;
        while ((pdb_prescaler1 != (uint32_t)-1) && (period < pdb_period_test)) {
            pdb_prescaler1--;
        ...
        */
        ...
        while ((pdb_prescaler != (uint32_t)-1) && (period < pdb_period_test)) {
            pdb_prescaler--;
        ...
        */
        ...
        pdb_external.prescaler = (int8_t) pdb_prescaler;
    }
    else {
    ...
    pdb_period1 = ((uint64_t) period << 16) / pdb_period;
...
_mqx_int _adt_hw_channel_init(MQX_FILE_PTR f, ADT_CHANNEL_PTR adt_ch)
{
    ...
    if (ch_num == 0) { /* temporary, we will make running channel A not actively, first trigger will move it to area within accesible boundary */
        pdb_internal.delayA[adc_num] = ((uint64_t) adt_ch->g.offset * PDB0_BASE_PTR->MOD) / pdb_period;/* <err001>: we must use pdb_period */
        PDB0_BASE_PTR->CH[adc_num].DLY[0] = (PDB0_BASE_PTR->MOD == PDB_MOD_MOD_MASK ? PDB_MOD_MOD_MASK : PDB0_BASE_PTR->MOD + 1); /* move ChA behind accessible boundary */
        adc_ch_bundle->waste_channel = 1;
    }
    else
        PDB0_BASE_PTR->CH[adc_num].DLY[ch_num] = ((uint64_t) adt_ch->g.offset * PDB0_BASE_PTR->MOD) / pdb_period;/* <err001>: we must use pdb_period */
...
*
*END*********************************************************************/
void *_bsp_get_pdb_base_address()
{
    return (void *) PDB0_BASE_PTR;
}
