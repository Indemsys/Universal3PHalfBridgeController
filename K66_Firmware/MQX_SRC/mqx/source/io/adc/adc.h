...
typedef struct adc_init_struct {
   uint32_t   resolution;       /* resolution of ADC driver, the bigger or equal possible value is used to configure ADC peripheral */
} ADC_INIT_STRUCT, * ADC_INIT_STRUCT_PTR;

typedef uint32_t ADT_TRIGGER_MASK;
...
typedef struct adc_init_channel_struct {
   uint16_t   source;           /* one of ADC_SOURCE_* definitions defined in ./<module_name>/adc_<module_name>.h - CPU specific header file */
   uint16_t   flags;            /* initialization flags, combination of ADC_CHANNEL_<FLAG_NAME> possibilities above */
   uint32_t   number_samples;   /* the number of samples needed to trigger event */
   uint32_t   time_offset;      /* the time offset applied once after measurement on given channel is started, in microseconds */
   uint32_t   time_period;      /* the time slot between two samples in microseconds */
   /* NOTE: the following item is not used in the current ADC driver and is prepared to be used in the future */
   uint32_t   range;            /* the range of result sample(s), NOT USED NOW */
   ...
   uint32_t            event_mask;     /* event mask to be signalled in lwevent after nuber_samples is measured */
#endif
} ADC_INIT_CHANNEL_STRUCT, * ADC_INIT_CHANNEL_STRUCT_PTR;

typedef uint32_t ADC_VALUE_TYPE;
typedef struct adc_result_struct {
   ADC_VALUE_TYPE   result;
} ADC_RESULT_STRUCT, * ADC_RESULT_STRUCT_PTR;
...
_mqx_uint _io_adc_install(char *, void *);
