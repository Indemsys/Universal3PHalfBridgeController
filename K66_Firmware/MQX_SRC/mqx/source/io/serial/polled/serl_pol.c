    ...
    )
{
    ...
    dev_ptr = _mem_alloc_system_zero((_mem_size)sizeof(IO_SERIAL_POLLED_DEVICE_STRUCT));
...
#endif
            (*polled_dev_ptr->DEV_DEINIT)(polled_dev_ptr->DEV_INIT_DATA_PTR, polled_dev_ptr->DEV_INFO_PTR);
#if MQX_ENABLE_LOW_POWER          
    ...
    )
{
    ...
    if (--polled_dev_ptr->COUNT == 0) {
        ...
        if (polled_dev_ptr->DEV_DEINIT) {
            ...
            _lwsem_wait (&(polled_dev_ptr->LPM_INFO.LOCK));
#endif
            result = (_mqx_int)(*polled_dev_ptr->DEV_DEINIT)(
                polled_dev_ptr->DEV_INIT_DATA_PTR, polled_dev_ptr->DEV_INFO_PTR
            );
#if MQX_ENABLE_LOW_POWER          
            _lwsem_post (&(polled_dev_ptr->LPM_INFO.LOCK));
#endif
    ...
    )
{
    ...
    while ( i ) {
        ...
        if (flags & IO_SERIAL_XON_XOFF) {
            ...
            while (
                (! _CHARQ_FULL(polled_dev_ptr->CHARQ)) && (*polled_dev_ptr->DEV_STATUS)(info_ptr)
            )
            {
                character = (unsigned char)(*polled_dev_ptr->DEV_GETC)(info_ptr);
                _CHARQ_ENQUEUE(polled_dev_ptr->CHARQ, character);
    ...
    switch (cmd) {
        case IO_IOCTL_CHAR_AVAIL:
            ...
            if ( _CHARQ_SIZE(polled_dev_ptr->CHARQ) ) {
                ...
                *bparam_ptr = (*polled_dev_ptr->DEV_STATUS)(polled_dev_ptr->DEV_INFO_PTR);
            }
            break;
        ...
        case IO_IOCTL_SERIAL_SET_FLAGS:
            ...
            if (polled_dev_ptr->DEV_IOCTL != NULL) {
                result = (*polled_dev_ptr->DEV_IOCTL)(polled_dev_ptr->DEV_INFO_PTR, cmd, param_ptr);
            }
            break;
