...
ASM_LABEL(fpu_restore_end)
                ...
                push {r1}
                push {lr}
                bl   ASM_PREFIX(_mqx_context_switch_internal)
                pop  {lr}
                pop  {r1}
#endif
...
ASM_LABEL(_isr_set_flag_return_end)
                orr r0, r0, #SWITCH_REQUEST_WHEN_TASK_PREEMPTION_DISABLED
                str r0, [r2, #TD_FLAGS]
                cpsie.n i
                pop {pc}
...
#if MQX_SAVE_FP_ALWAYS && PSP_HAS_FPU
                /* destination stack address (4B + 32B + 128B - exception stack frame + floating registers context) */
                sub r12, r12, #0x88
#else
                /* destination stack address (4B + 32B - exception stack frame) */
                sub r12, r12, #0x04   /*#36*/
#endif
                and r7, r7, #0xFFFFFDFF /* clear 9 bit of XPSR to avoid HW aligner */
                stmdb r12!, {r0-r7}
...
#else
/* Beloved CW10 dont know strexb. This workaround could cause warning: "Mismatch between the condition and the previous IT instruction... ". It is not an issue. */
 ASM_CONST16(0xe8c0)
 ASM_CONST16(0x2f43)
#endif
