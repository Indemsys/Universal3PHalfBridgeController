...
/*
** When MQX_USE_IO_OLD is defined as 1, the MQX implements the I/O subsystem calls
...
/*
** MQX ASSERT define has to be set to some value
** defined in mqx.h.
** MGCT: <option type="uint32_t"/>
 */
#ifndef MQX_ASSERT
#define MQX_ASSERT  MQX_ASSERT_LOOP
#endif
...
/*
** When MQX_HAS_CONTEXT_SWITCH_HANDLER is defined as 1, MQX includes code to execute context
** switch handler during the context switching. Also the _mqx_set_context_switch_handler
** call is also included.
** MGCT: <option type="bool"/>
*/
#ifndef MQX_HAS_CONTEXT_SWITCH_HANDLER
#define MQX_HAS_CONTEXT_SWITCH_HANDLER 1
#endif
...
#if !MQX_USE_LWMEM_ALLOCATOR && !MQX_USE_MEM && !MQX_USE_TLSF_ALLOCATOR
#error Set MQX_USE_MEM to 1 when not using MQX_USE_LWMEM_ALLOCATOR nor MQX_USE_TLSF_ALLOCATOR
#endif

#if MQX_USE_LWMEM_ALLOCATOR && MQX_USE_TLSF_ALLOCATOR
#error Cannot use TLSF and LWMEM allocators in parallel, set only one of MQX_USE_LWMEM_ALLOCATOR and MQX_USE_TLSF_ALLOCATOR to 1
#endif
