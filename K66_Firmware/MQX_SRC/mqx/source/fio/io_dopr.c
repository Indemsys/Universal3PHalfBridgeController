...
extern void _io_doprint_prt(int32_t, unsigned char *, int32_t, bool);
extern void _io_doprint_prt64(int64_t, unsigned char *, int32_t, bool);

#if MQX_INCLUDE_FLOATING_POINT_IO
extern _mqx_int _io_dtoe(char *, double arg, char *, char, char, char, int32_t, char);
extern _mqx_int _io_dtof(char *, double arg, char *, char, char, char, int32_t, char);
extern _mqx_int _io_dtog(char *, double arg, char *, char, char, char, int32_t, char);
#endif
...
static _mqx_int _io_putstr(MQX_FILE_PTR, IO_PUTCHAR_FPTR func_ptr, _mqx_int max_count, char *str);
      ...
      switch (  f  ) {
         ...
         case 'U':
            ...
            if (arg_length == LONGLONG) {
               ...
               _io_doprint_prt64(i64, str_ptr + 1, PRINT_DECIMAL_64, (bool)use_caps);
               str_ptr[1] += digit1;
               if (str_ptr[1] > '9') {
                  str_ptr[1] -= 10;
                  str_ptr[0] = '1';
               ...
               _io_doprint_prt(i, str_ptr, PRINT_DECIMAL, (bool)use_caps);
               str_ptr[0] += digit1;
            }
            use_prep = 0;
            break;
         ...
         case 'O':
            ...
            if (arg_length == LONGLONG) {
               i64 = va_arg(args_ptr, uint64_t);
               zero_value = i64 == 0L;
               _io_doprint_prt64(i64, str_ptr, PRINT_OCTAL_64, (bool)use_caps);
            } else {
               if (arg_length == HALF) {
                  i = (uint16_t) va_arg(args_ptr, unsigned int);
               } else if (arg_length == HALFHALF) {
               ...
               zero_value = i == 0;
               _io_doprint_prt(i, str_ptr, PRINT_OCTAL, (bool)use_caps);
            }
            prepend = 3;
            break;
         ...
         case 'x':
            ...
            if (arg_length == LONGLONG) {
               i64 = va_arg(args_ptr, uint64_t);
               zero_value = i64 == 0L;
               _io_doprint_prt64(i64, str_ptr, PRINT_HEX_64, (bool)use_caps);
            } else {
               if (arg_length == HALF) {
                  i = (uint16_t) va_arg(args_ptr, unsigned int);
               } else if (arg_length == HALFHALF) {
               ...
               zero_value = i == 0;
               _io_doprint_prt(i, str_ptr, PRINT_HEX, (bool)use_caps);
            }
            break;
         ...
         case 'N':   /* Defaults to size of number on stack */
            str_ptr = (unsigned char *)va_arg(args_ptr, void *);
            *(_mqx_int *)(str_ptr) = (int32_t)number_of_characters;
            continue;
