...
{
   register int32_t x;       /* Partial result */
   register int32_t exp;     /* Power of 10 for the result */
   register double result;
   register _mqx_int ne;     /* Number of digits seen so far */
   register _mqx_int nx;     /* Number of didits used so far */
   register int32_t eminus, minus;
   register char  *p;
...
static const unsigned char _io_modf_masktable[9] =
...
{ /* Body */
   ...
   if (intpart != 0) {
      ...
      while (intpart != 0) {
         ...
         tmpintpart = modf(tmpintpart, (double *)(&intpart));
