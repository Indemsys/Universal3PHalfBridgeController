...
{ /* Body */
    ...
    if ( ((msgq_ptr->TD_PTR == NULL) || (msgq_ptr->TD_PTR == td_ptr)))
    {
        ...
        while ( imsg_ptr != NULL )
        {
            ...
            _msg_free((void *)&imsg_ptr->MESSAGE);
        ...
        if ( msgq_ptr->TD_PTR )
        {
            ...
            {
                td_ptr->MSG_QUEUE_HEAD = (void *)td_msgq_ptr->NEXT_MSGQ_PTR;
            ...
            else
            {
                ...
                if ( td_ptr->MSG_QUEUE_TAIL == (void *)td_msgq_ptr )
                {
                    td_ptr->MSG_QUEUE_TAIL = (void *)qprev_ptr;
                } /* Endif */
            } /* Endif */
        } /* Endif */
        msgq_ptr->QUEUE = 0;
...
{ /* Body */
    ...
    result = _msgq_open_internal(
        queue, max_queue_size, MSG_QUEUE, (MSGQ_NOTIFICATION_FPTR)NULL, NULL
    );
...
{ /* Body */
    ...
    msgq_ptr->TYPE = (uint16_t)queue_type;
    ...
    if ( queue_type == MSG_QUEUE )
    {
        ...
        if ( td_ptr->MSG_QUEUE_HEAD == NULL )
        {
            td_ptr->MSG_QUEUE_HEAD = (void *)msgq_ptr;
            td_ptr->MSG_QUEUE_TAIL = (void *)msgq_ptr;
...
{ /* Body */
    ...
    else
    {
        ...
        _INT_ENABLE();
        _KLOGX5(KLOG_msgq_peek, message_ptr, message_ptr->TARGET_QID, message_ptr->SOURCE_QID, *(_mqx_uint_ptr)((unsigned char *)message_ptr+sizeof(MESSAGE_HEADER_STRUCT)));
    } /* Endif */
    return (void *)message_ptr;
...
{ /* Body */
    ...
    else if (error == MQX_OK)
    {
        _KLOGX5(KLOG_msgq_receive, message_ptr, message_ptr->TARGET_QID,
                        message_ptr->SOURCE_QID, *(_mqx_uint_ptr)((unsigned char *)message_ptr+
...
{ /* Body */
    ...
    *error_ptr = MQX_OK;
    ...
    {
        _KLOGX2(KLOG_msgq_receive_internal, MQX_CANNOT_CALL_FUNCTION_FROM_ISR);
...
{ /* Body */
    ...
    if (time_in_ticks)
    {
        ...
        message_ptr = _msgq_receive_internal(queue_id, &ticks, MSG_TIMEOUT_RELATIVE, &error);
    }
    else
    {
        message_ptr = _msgq_receive_internal(queue_id, &ticks, MSG_TIMEOUT_NONE, &error);
    } /* Endif */
 ...
 */ 
bool _msgq_send_blocked_internal
(
    void   *input_msg_ptr
)
{ /* Body */
