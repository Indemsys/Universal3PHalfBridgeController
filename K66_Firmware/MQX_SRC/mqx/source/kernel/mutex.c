...
{ /* Body */
    ...
    {
        _KLOGX2(KLOG_mutex_destroy, MQX_CANNOT_CALL_FUNCTION_FROM_ISR);
...
{ /* Body */
    ...
    if (!((protocol == MUTEX_PRIO_INHERIT) || (protocol == MUTEX_PRIO_PROTECT) || (protocol == MUTEX_NO_PRIO_INHERIT) || (protocol == (MUTEX_PRIO_INHERIT | MUTEX_PRIO_PROTECT))))
    {
        _KLOGX2(KLOG_mutatr_set_sched_protocol, MQX_EINVAL);
        return (MQX_EINVAL);
    } /* Endif */
...
{ /* Body */
    ...
    if (kernel_data->IN_ISR)
    {
        _KLOGX2(KLOG_mutex_test, MQX_CANNOT_CALL_FUNCTION_FROM_ISR);
        return (MQX_CANNOT_CALL_FUNCTION_FROM_ISR);
    } /* Endif */
