 ...
 */
volatile uint32_t _tad_task_at_flag;
...
)
{
    ...
    if (td_ptr == kernel_data->ACTIVE_PTR)
    {
        ...
        {
            stack_ptr = (void *) td_ptr->STACK_PTR;
...
{
    MQX_API_CALL_PARAMS params = {(uint32_t)task_id, 0, 0, 0, 0};
...
)
{
    MQX_API_CALL_PARAMS params =
        {(uint32_t)processor_number, (uint32_t)template_index, (uint32_t)parameter, 0, 0};
...
{ /* Body */
    ...
    _KLOGX3(KLOG_task_create_block, result, kernel_data->ACTIVE_PTR->TASK_ERROR_CODE);
...
{ /* Body */
    ...
    _KLOGE5(KLOG_task_create_at, processor_number, template_index, parameter, stack_ptr);
    ...
    _KLOGX3(KLOG_task_create_at, result, kernel_data->ACTIVE_PTR->TASK_ERROR_CODE);
...
)
{
    ...
    td_ptr = kernel_data->ACTIVE_PTR;
        ...
        void   *next_block_ptr;
        void   *block_ptr = victim_ptr->FIRST_OWNED_BLOCK;
        TD_STRUCT_PTR system_td_ptr = SYSTEM_TD_PTR(kernel_data);

        while (block_ptr != NULL)
        {
            next_block_ptr = ((tlsf_adaptation_structure_t*)block_ptr)->next;
            /* Make an offset back to the user data chunk */
            block_ptr = tlsf_get_ptr_from_adaptation((void*)block_ptr);
            /* dealloc everything else except td, stack and floating point context. */

            if (td_ptr != victim_ptr)
            {
                _mem_transfer_internal(block_ptr, system_td_ptr);
            }

            if
                (
                    (block_ptr != victim_ptr) &&
                    (block_ptr != victim_ptr->STACK_ALLOC_BLOCK) &&
                    (block_ptr != victim_ptr->FLOAT_CONTEXT_PTR)
                )
            {
                _mem_free(block_ptr);
            }
    ...
    #else
    {
        void   *block_ptr = _mem_get_next_block_internal(victim_ptr, NULL);

        while (block_ptr != NULL)
        {
            void   *next_block_ptr = _mem_get_next_block_internal(victim_ptr, block_ptr);
            /* This would cause either a double free or switching to system tasks
               resource list, which causes all systems resource deallocation, which
               consequently results in fatal system failure.*/
       
            assert(block_ptr != next_block_ptr);  
            if (td_ptr != victim_ptr)
            {
    ...
    if (victim_ptr == td_ptr) {
        ...
        if (victim_ptr->FLOAT_CONTEXT_PTR != NULL)
        {
            _mem_transfer_internal(victim_ptr->FLOAT_CONTEXT_PTR, SYSTEM_TD_PTR(kernel_data));
 ...
 * \see TD_STRUCT
 */
uint32_t _task_get_parameter_internal
(
    TD_STRUCT_PTR td_ptr
)
{
 ...
 * \see TD_STRUCT
 */
uint32_t _task_set_parameter_internal
(
    uint32_t       new_value,
    TD_STRUCT_PTR td_ptr
)
    ...
    *priority_ptr = td_ptr->HOME_QUEUE->PRIORITY;
    ...
    if (td_ptr->STATE == TASK_QUEUE_BLOCKED)
    {
        task_queue_ptr = (TASK_QUEUE_STRUCT_PTR)
                         ((unsigned char *)td_ptr->INFO - FIELD_OFFSET(TASK_QUEUE_STRUCT, TD_QUEUE));
...
{
    MQX_API_CALL_PARAMS params = {(uint32_t)td, 0, 0, 0, 0};
...
{ /* Body */
    ...
    #if MQX_ALLOCATOR_GARBAGE_COLLECTING
    {
        ...
        block_ptr = victim_ptr->FIRST_OWNED_BLOCK;
        ...
        while (block_ptr != NULL)
        {
            ...
            /* Make an offset back to the user data chunk */
            block_ptr = tlsf_get_ptr_from_adaptation((void*)block_ptr);
            if (!(block_ptr == victim_ptr || MEM_TYPE_TD == GET_MEMBLOCK_TYPE(block_ptr)
    ...
    if (victim_ptr->FLAGS & TASK_STACK_PREALLOCATED) {
        victim_ptr->FLAGS = template_ptr->TASK_ATTRIBUTES | TASK_STACK_PREALLOCATED;
    }
    else {
 ...
 * \brief Enables preemption of the current task.
 *
 * The _task_enable_preemption() function enables preemption of the active task
 * after _task_stop_preemption() was called, this function invokes _CHECK_RUN_SCHEDULER()
 * only if SWITCH_REQUEST_WHEN_TASK_PREEMPTION_DISABLED flag is set.
 *
 * \warning Changes the preemption ability of tasks.
 * \warning Interrupts are still handled.
 *
 ...
 * \see _task_block
 */
void _task_allow_preemption(void)
{ /* Body */
    register KERNEL_DATA_STRUCT_PTR kernel_data;
    TD_STRUCT_PTR                   td_ptr;

    _GET_KERNEL_DATA(kernel_data);
    _KLOGE1(KLOG_task_enable_preemption);

    td_ptr = kernel_data->ACTIVE_PTR;
    _int_disable();
    td_ptr->FLAGS &= ~TASK_PREEMPTION_DISABLED;
    if(td_ptr->FLAGS & SWITCH_REQUEST_WHEN_TASK_PREEMPTION_DISABLED)
    {
        td_ptr->FLAGS &= ~SWITCH_REQUEST_WHEN_TASK_PREEMPTION_DISABLED;
        _CHECK_RUN_SCHEDULER();
    }
    _int_enable();

    _KLOGX1(KLOG_task_enable_preemption);
...
{ /* Body */
    ...
    if ( &loc < ((char *)kernel_data->ACTIVE_PTR->STACK_LIMIT) )
    {
#else
        if ( &loc > ((char *)kernel_data->ACTIVE_PTR->STACK_LIMIT) )
        {
...
)
{
    ...
    if (task_id == MQX_NULL_TASK_ID)
    {
        return((void *)kernel_data->ACTIVE_PTR);
    }
 ...
 * \see _usr_task_ready
 */
void *_usr_task_get_td
(
    _task_id task_id
)
{
    MQX_API_CALL_PARAMS params = {(uint32_t)task_id, 0, 0, 0, 0};
    return (void *)_mqx_api_call(MQX_API_TASK_GET_TD, &params);
}
...
{
    MQX_API_CALL_PARAMS params = {(uint32_t)new_error_code, 0, 0, 0, 0};
 ...
 */
void *_task_get_reserved_base(void)
{ /* Body */
    KERNEL_DATA_STRUCT_PTR kernel_data;
    _GET_KERNEL_DATA(kernel_data);
...
{ /* Body */
    ...
    if (template_ptr->TASK_PRIORITY > kernel_data->LOWEST_TASK_PRIORITY)
    {
        ...
        if ((template_ptr != &kernel_data->INIT_TASK_TEMPLATE) && (template_ptr != &kernel_data->IDLE_TASK_TEMPLATE))
#else
        if (template_ptr != &kernel_data->INIT_TASK_TEMPLATE)
    ...
    if (dynamic)
    {
        ...
        new_template_ptr = (void *)stack_ptr;
        stack_ptr +=  sizeof(TASK_TEMPLATE_STRUCT);
    ...
    {
#if MQX_CHECK_MEMORY_ALLOCATION_ERRORS
        _mem_free(td_ptr->STACK_ALLOC_BLOCK);
        _mem_free(td_ptr);
#endif
        /* build stack failed */
        return NULL;
     ...
     * newly created task.
     */
        _mem_transfer_td_internal(td_ptr->STACK_ALLOC_BLOCK, kernel_data->ACTIVE_PTR, td_ptr);
        _mem_transfer_td_internal(td_ptr, kernel_data->ACTIVE_PTR, td_ptr);
...
{ /* Body */
     ...
     * on it.
     */
    if ( (unsigned char *)ready_q_ptr > (unsigned char *)(kernel_data->CURRENT_READY_Q) )
    {
...
{ /* Body */
    ...
    (void)  stack_ptr;  /* suppress 'unused variable' warning */
...
{ /* Body */
    ...
    if (tos != 0)
    {
        uint32_t align_mask = kernel_data->TOS_RESERVED_ALIGN_MASK;
    ...
    *overhead = extra;
    ...
    if (input_stack_ptr)
    {
        ...
        _mem_zero((void *)new_td_ptr, extra);
    }
#if !MQX_LITE_VERSION_NUMBER
    else
    {
...
#if MQX_USE_TLSF_ALLOCATOR
    #if MQX_ALLOCATOR_GARBAGE_COLLECTING
            new_td_ptr->FIRST_OWNED_BLOCK = NULL;
    #endif
#endif
        }
    ...
    if (tos != 0)
    {
        new_td_ptr->TOS_RESERVED = ((unsigned char *) new_td_ptr) + tos_offset;
    } /* Endif */
#endif
    ...
    if (!insert_td_ptr)
    {
        q_ptr = (QUEUE_ELEMENT_STRUCT_PTR)((void *)&kernel_data->TD_LIST);
    }
    else
    {
    ...
    do
    {
        ...
        next_td_ptr   = (TD_STRUCT_PTR)((unsigned char *)insert_td_ptr->TD_LIST_INFO.NEXT - FIELD_OFFSET(TD_STRUCT,TD_LIST_INFO));
        prev_td_ptr   = next_td_ptr;
        INC_TASK_NUMBER(task_num);
    } while( ((void *)next_td_ptr != ((void *)&kernel_data->TD_LIST)) &&
...
#if MQX_MEM_MONITOR
_mqx_uint _task_stack_usage
(
    _task_id            task_id,
    _mem_size*          stack_size_ptr,
    _mem_size*          stack_used_ptr
)
{
...
_mqx_uint _task_int_stack_usage
(
