 ...
 * _sem_create().
 * \param[in] force_destroy A bool flag, see Description.
 * 
 * \return MQX_OK
 * \return MQX_CANNOT_CALL_FUNCTION_FROM_ISR (Function cannot be called from an ISR.)
...
{ /* Body */
    ...
    *returned_sem_ptr = (void *)sem_connection_ptr;
...
{ /* Body */
    ...
    conn_prev_ptr = (SEM_CONNECTION_STRUCT_PTR)((void *)queue_ptr);
    conn2_ptr = (SEM_CONNECTION_STRUCT_PTR)((void *)queue_ptr->NEXT);
...
{ /* Body */
    ...
    bool                   destroying_semaphore = FALSE;
    ...
    if (_QUEUE_GET_SIZE(&sem_ptr->WAITING_TASKS))
    {
        ...
        ((void *)sem_ptr->WAITING_TASKS.NEXT);
        while (new_sem_connection_ptr != (void *)&sem_ptr->WAITING_TASKS)
        {
    ...
    while (connection_ptr)
    {
                        ...
                        (connection_ptr->TD_PTR == td_ptr) )
        {
            ...
            if (sem_ptr->VALID == SEM_VALID)
            {
                ...
                sem_connection_ptr = (SEM_CONNECTION_STRUCT_PTR)
                ((void *)sem_ptr->WAITING_TASKS.NEXT);
                while (sem_connection_ptr != (void *)&sem_ptr->WAITING_TASKS.NEXT)
                {
...
{ /* Body */
    ...
    if (kernel_data->IN_ISR)
    {
        _KLOGX2(KLOG_sem_wait_internal, MQX_CANNOT_CALL_FUNCTION_FROM_ISR);
        return(MQX_CANNOT_CALL_FUNCTION_FROM_ISR);
    } /* Endif */
#endif
    ...
    if (sem_ptr->COUNT == 0)
    {
        ...
        if (sem_ptr->POLICY & SEM_STRICT)
        {
                            ...
                            (_QUEUE_GET_SIZE(&sem_ptr->OWNING_TASKS) > 0))
            {
                ...
                ((void *)sem_ptr->OWNING_TASKS.NEXT);
                saved_connection_ptr = list_connection_ptr;
                priority = kernel_data->LOWEST_TASK_PRIORITY;
                while ((void *)list_connection_ptr !=
                                (void *)&sem_ptr->OWNING_TASKS)
                {
