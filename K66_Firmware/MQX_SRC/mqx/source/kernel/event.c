...
{ /* Body */
    ...
    _KLOGE4(KLOG_event_wait_any_ticks, users_event_ptr, bit_mask, timeout_in_ticks);
...
(
    char              *name_ptr,
    EVENT_STRUCT_PTR  *event_ptr_ptr
)
{ /* Body */
    KERNEL_DATA_STRUCT_PTR              kernel_data;
    register EVENT_COMPONENT_STRUCT_PTR event_component_ptr;
...
{ /* Body */
    ...
    while (--i)
    {
        event_connection_ptr = (EVENT_CONNECTION_STRUCT_PTR) ((void *) queue_ptr->NEXT);
...
{ /* Body */
    ...
    *returned_event_ptr = (void *) event_connection_ptr;
    ...
    *returned_event_ptr = NULL;
    ...
    if (processor != 0)
    {
        ...
        if (kernel_data->IPC)
        {
            ...
            event_connection_ptr->EVENT_PTR = (void *)
...
{ /* Body */
    ...
    if (_QUEUE_GET_SIZE(&event_ptr->WAITING_TASKS))
    {
        ...
        conn_ptr = (EVENT_CONNECTION_STRUCT_PTR) ((void *) event_ptr->WAITING_TASKS.NEXT);
        while (conn_ptr != (EVENT_CONNECTION_STRUCT_PTR) ((void *) &event_ptr->WAITING_TASKS))
        {
    ...
    while (connection_ptr)
    {
        if ((connection_ptr->VALID == EVENT_VALID) && (connection_ptr->TD_PTR == td_ptr))
        {
            ...
            if (event_ptr->VALID == EVENT_VALID)
            {
                ...
                _int_disable();
                event_connection_ptr = (EVENT_CONNECTION_STRUCT_PTR) ((void *) event_ptr->WAITING_TASKS.NEXT);
                while (event_connection_ptr != (void *) &event_ptr->WAITING_TASKS.NEXT)
                {
...
{ /* Body */
    ...
    if (kernel_data->IN_ISR)
    {
        _KLOGX2(KLOG_event_test, MQX_CANNOT_CALL_FUNCTION_FROM_ISR);
        return (MQX_CANNOT_CALL_FUNCTION_FROM_ISR);
    } /* Endif */
    ...
    *event_error_ptr = NULL;
    ...
    result = _name_test_internal((NAME_COMPONENT_STRUCT_PTR) event_component_ptr->NAME_TABLE_HANDLE,
                    (void **) &table_ptr, (void **) event_error_ptr);
    _int_enable();
    if (result != MQX_OK)
    {
