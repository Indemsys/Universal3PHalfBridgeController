 ...
 * the partition block (for dynamic partitions only).
 */
void *_partition_alloc_internal
(
...
{ /* Body */
    ...
    _INT_DISABLE(); // uses kernel data - based on DEFINE combination
    ...
    if (partpool_ptr->AVAILABLE == 0)
    {
        ...
        if (((volatile PARTPOOL_STRUCT *) partpool_ptr)->AVAILABLE == 0)
        {
            _int_enable();
    ...
    return (void *) ((unsigned char *) block_ptr + sizeof(INTERNAL_PARTITION_BLOCK_STRUCT));
...
{ /* Body */
    ...
    PARTPOOL_STRUCT_PTR partpool_ptr;
    unsigned char       *block_area_ptr;
    _mem_size           block_area_size;
    _mqx_uint           number_of_blocks;
    _mqx_uint           result;
    ...
    partpool_ptr = (PARTPOOL_STRUCT_PTR)_ALIGN_ADDR_TO_HIGHER_MEM(partition_location);
    block_area_ptr = (unsigned char *) partpool_ptr + sizeof(PARTPOOL_STRUCT);
    block_area_ptr = (unsigned char *) _ALIGN_ADDR_TO_HIGHER_MEM(block_area_ptr);

#if MQX_CHECK_ERRORS
    if (partition_size < (_mem_size) block_area_ptr - (_mem_size) partition_location)
    {
        _task_set_error(MQX_INVALID_PARAMETER);
        _KLOGX2(KLOG_partition_create_at, PARTITION_NULL_ID);
        return (PARTITION_NULL_ID);
...
#endif /* MQX_CHECK_ERRORS */
    block_area_size = partition_size - ((_mem_size) block_area_ptr - (_mem_size) partition_location);
    block_size += (_mem_size) sizeof(INTERNAL_PARTITION_BLOCK_STRUCT);
    _MEMORY_ALIGN_VAL_LARGER(block_size);

    number_of_blocks = block_area_size / block_size;
#if MQX_CHECK_ERRORS
...
{ /* Body */
    ...
    block_ptr = (INTERNAL_PARTITION_BLOCK_STRUCT_PTR)((unsigned char *) partpool_ptr + sizeof(PARTPOOL_STRUCT));
    ...
    while (initial_blocks--)
    {
        ...
        block_ptr = (INTERNAL_PARTITION_BLOCK_STRUCT_PTR)((unsigned char *) block_ptr + actual_size);
    } /* Endwhile */
...
{ /* Body */
    ...
    block_ptr = (unsigned char *) partpool_block_ptr + sizeof(PARTPOOL_BLOCK_STRUCT);
    block_ptr = (unsigned char *) _ALIGN_ADDR_TO_HIGHER_MEM(block_ptr);

#if MQX_CHECK_ERRORS
    if (partition_size < (_mem_size) block_ptr - (_mem_size) partition_location)
    {
        _task_set_error(MQX_INVALID_PARAMETER);
        _KLOGX2(KLOG_partition_extend, MQX_INVALID_PARAMETER);
        return (MQX_INVALID_PARAMETER);
...
#endif /* MQX_CHECK_ERRORS */
    block_area_size = partition_size - ((_mem_size) block_ptr - (_mem_size) partition_location);
    number_of_blocks = block_area_size / partpool_ptr->BLOCK_SIZE;
#if MQX_CHECK_ERRORS
    if (number_of_blocks == 0)
    {
        _task_set_error(MQX_INVALID_PARAMETER);
...
{ /* Body */
    ...
    block_ptr = (INTERNAL_PARTITION_BLOCK_STRUCT_PTR)((unsigned char *) partpool_block_ptr + sizeof(PARTPOOL_BLOCK_STRUCT));
    ...
    while (number_of_blocks--)
    {
        ...
        partpool_ptr->AVAILABLE++;
        _int_enable();
        block_ptr = (INTERNAL_PARTITION_BLOCK_STRUCT_PTR)((unsigned char *) block_ptr + partpool_ptr->BLOCK_SIZE);
    } /* Endwhile */
...
{ /* Body */
    ...
    partpool_ptr = (PARTPOOL_STRUCT_PTR)((void *) part_component_ptr->PARTITIONS.NEXT);
    while (partpool_ptr != (PARTPOOL_STRUCT_PTR)((void *) &part_component_ptr->PARTITIONS))
    {
...
{ /* Body */
    ...
    if (kernel_data->IN_ISR)
    {
        _KLOGX2(KLOG_partition_test, MQX_CANNOT_CALL_FUNCTION_FROM_ISR);
        return (MQX_CANNOT_CALL_FUNCTION_FROM_ISR);
    } /* Endif */
#endif /* MQX_CHECK_ERRORS */
        ...
        while (partpool_block_ptr != NULL)
        {
            ...
            *partpool_block_in_error = partpool_block_ptr;
            ...
            while (--i)
            {
                ...
                {
                    _int_enable();
                    *block_in_error = (void *) ((unsigned char *) block_ptr + sizeof(INTERNAL_PARTITION_BLOCK_STRUCT));
...
{ /* Body */
    ...
    while (partpool_block_ptr != NULL)
    {
        tmp_ptr = (unsigned char *) partpool_block_ptr + sizeof(PARTPOOL_BLOCK_STRUCT);
        tmp_ptr = (unsigned char *) _ALIGN_ADDR_TO_HIGHER_MEM(tmp_ptr);
        size += (_mem_size) (tmp_ptr - (unsigned char *) partpool_block_ptr);
        partpool_block_ptr = partpool_block_ptr->NEXT_POOL_PTR;
    } /* Endif */

    tmp_ptr = (unsigned char *) partition + sizeof(PARTPOOL_STRUCT);
    tmp_ptr = (unsigned char *) _ALIGN_ADDR_TO_HIGHER_MEM(tmp_ptr);

    size += (_mem_size) (tmp_ptr - (unsigned char *) partition);
...
{ /* Body */
    ...
    _KLOGM(_GET_KERNEL_DATA(kernel_data));
    _KLOGE3(KLOG_partition_calculate_blocks, partition_size, block_size);
    ...
    {
        _task_set_error(MQX_INVALID_PARAMETER);
        _KLOGX2(KLOG_partition_calculate_blocks, PARTITION_NULL_ID);
        return (0);
    }
#endif /* MQX_CHECK_ERRORS */

    blocks = (partition_size - size) / actual_size;

    _KLOGX2(KLOG_partition_calculate_blocks, blocks);
