...
static unsigned char inbuf[SNMPCFG_BUFFER_SIZE];
static unsigned char outbuf[SNMPCFG_BUFFER_SIZE];

void SNMP_task (void *, void *);
...
static bool SNMP_parse_varbindlist
   (
      SNMP_PARSE_PTR snmp,
      uint32_t    *writelen
   )
{ /* Body */
   unsigned char   *outp, *varp;
   uint32_t     outlen, varlen, varcount, index;
...
static bool SNMP_parse_awesome_pdubulk
   (
      SNMP_PARSE_PTR snmp
   )
{ /* Body */
...
static bool SNMP_parse
   (
      SNMP_PARSE_PTR snmp,
      uint32_t    *writelen
   )
{ /* Body */
   uint32_t     typelen, maxheadlen;
   uint32_t     ctylen, idlen;
   unsigned char   *ctyp,   *idp,  *pdup;
...
{ /* Body */
   SNMP_PARSE        snmp;
   sockaddr_in       addr;
   uint32_t           sock, error, count;
   int32_t            inlen, j;
   uint32_t           outlen,time;
   uint16_t           addrlen;
   uint32_t           option;
   char          *community_array[] = SNMPCFG_COMMUNITY_LIST;
   _ip_address      *trap_receiver_list_ptr = (_ip_address *)traps;
   ...
   _task_set_environment( _task_get_id(), (void *) &snmp_context );
   ...
   for (;;) {
      ...
      addrlen = sizeof(addr);
      inlen = recvfrom(sock, inbuf, SNMPCFG_BUFFER_SIZE, 0, (sockaddr*)&addr, &addrlen);
      ...
      if (SNMP_parse(&snmp, &outlen)) {
         ...
         sendto(sock, snmp.outbuf, outlen, 0, (sockaddr*)&addr, addrlen);
...
#endif
         sendto(sock, inbuf, inlen, 0, (sockaddr*)&addr, addrlen);
      } else {
