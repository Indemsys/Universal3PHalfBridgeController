...
{ /* Body */
   ...
   ** care of because we're using uint32_t's.
   **
   ...
   **    but -11DD becomes 11DD as uint32_t.
   */
   ...
   if ((uint32_t)(tcb->rcvnxt - tcb->rcvuna) >= tcb->rcv2full/* || tcb->status & TCPS_RPUSH*/) {
#if 0
...
void TCP_Advance_receive_user
   (
      ...
      int32_t         size  /* IN     - no. of bytes given to user */
   )
{ /* Body */
...
void TCP_Truncate_receive_chunks
   (
      ...
      int32_t               datalen  /* IN     - length of data to remove */
   )
{ /* Body */
   Rchunk        *chunk;
...
int32_t TCP_Advertise_receive_window
   (
      TCB_STRUCT_PTR       tcb,     /* IN/OUT - TCP layer context */
      TCP_CFG_STRUCT_PTR   tcp_cfg  /* IN/OUT - TCP layer data */
   )
{ /* Body */
   ...
   if ( increment > (uint32_t)tcb->sndmax ) {
      increment = (uint32_t)tcb->sndmax;
   } /* Endif */
   ...
   **    but -D623 becomes D623 as uint32_t.
   */
   if ( (uint32_t)(newedge - tcb->rcvwndedge) >= (increment << 1) ) {
      tcb->rcvwndedge = newedge;
      TCP_Must_send_ack(tcb, tcp_cfg);
      ack_sent = 1;

   } else if ( (uint32_t)(newedge - tcb->rcvwndedge) >= increment ) {
...
void TCP_Start_receiving
   (
      TCB_STRUCT_PTR       tcb,     /* IN/OUT - TCP layer context */
      uint32_t              irs,     /* IN     - initial receive sequence */
      TCP_CFG_STRUCT_PTR   tcp_cfg  /* IN/OUT - TCP layer data */
   )
{ /* Body */
   ...
   (void)TCP_Advertise_receive_window(tcb, tcp_cfg);
   tcb->sndwl1 = irs;
...
{ /* Body */
   ...
   */
   ...
   if (req->ERROR_CODE == RTCSERR_TCP_TIMED_OUT)
   {
       req->OPCODE = tcb->rcvnxt - req->OPCODE; /* OPCODE is set to tcb->rcvbufseq when app issues receive request */
   }
...
{ /* Body */
   bool  ret_state = TRUE; /* Default return status to TRUE */
...
{ /* Body */
   int32_t   n;                /* no. of received bytes available */
   bool  ret_state = TRUE; /* Default return status to TRUE */
   int32_t   result = RTCS_OK;
               ...
               (tcb->rcvevents & TCPO_PUSH) == 0)
            {
               ...
               */
               ...
               if ( tcp_cfg->Rchunkfree != NULL ) {
                  ...
                  chunk = (Rchunk *)RTCS_mem_alloc_zero(sizeof(Rchunk));
               } /* Endif */
...
{ /* Body */
   ...
   {
     tcp_req->BUFF_LENG = 0;
     RTCSCMD_complete(tcp_req, RTCSERR_SOCK_INVALID);
     return;
   }   
...
{ /* Body */
   ...
   */
   dataseq = mqx_ntohl(seg->seq);
   if ( mqx_ntohs(seg->flags) & SYN ) {       /* first byte is the SYN */
      dataseq++;
   } /* Endif */
   ...
   */
   size     -= truncate;
   dataseq  += truncate;
   /* this code expects that data will be continuous.
      possibly the PCB can contain multiple fragments, so we use RTCSPCB_memcopy
      function to copy these fragments out of PCB to build one continuous data.
      This function is used also in UDP code.
      */
   unfrag_data = RTCS_mem_alloc_system(size + truncate);
   if(NULL == unfrag_data)
   {
     /* drop these data - we have not yet ack'ed the data, so remote peer will retransmit */
     RTCSLOG_FNX2(RTCSLOG_FN_TCP_Process_data, tcb);
     TCP_Must_send_ack(tcb, tcp_cfg);
     return;
   }
   RTCSPCB_memcopy(pcb, unfrag_data, 0, size + truncate);

   /* pointer to new data */
   data = unfrag_data + truncate;
   ...
   */
   ...
   if ( mqx_ntohs(seg->flags) & (PSH | FIN) ) {
      if ( (tcb->status & TCPS_RPUSH) == 0 ||
   ...
   */
   if ( GT32(tcb->rcvbufseq + n, tcb->rcvnxt) ) { /* rcvd new contiguous data */
      ...
      DEBUGTCP(printf("\nTCP_ProcDta: Call to AdRcvwnd: rcvwndedge = %X, rcvnxt = %X, wind = %d, rbs = %X ",
                     tcb->rcvwndedge, tcb->rcvnxt,
                     tcb->rcvwndedge - tcb->rcvnxt, tcb->rcvbufseq));
      ack_sent = TCP_Advertise_receive_window(tcb, tcp_cfg);
      if(0 == ack_sent)
      {
        TCP_Must_send_ack(tcb, tcp_cfg);
      }
   ...
   */
   ...
   if (GT32(tcb->rcvnxt, tcb->rcvbufseq)) {
      SOCK_select_signal(tcb->SOCKET, tcb->state, RTCS_OK);
   } /* Endif */
