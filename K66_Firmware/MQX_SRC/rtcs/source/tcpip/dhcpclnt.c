...
uint32_t DHCPCLNT_probe_address
   (
...
#if RTCSCFG_ENABLE_IP4
static uint32_t DHCP_send_internal
   (
...
{ /* Body */
   ...
   */
   ...
   dhcp_ptr->HANDLE             = (IP_IF *)parms->HANDLE;
   ...
   dhcp_ptr->PACKET             = (unsigned char *)(dhcp_ptr + 1);
...
{ /* Body */
   DHCP_HEADER           *header_ptr;
   uint32_t                xid;

   header_ptr = (DHCP_HEADER *)dhcp_ptr->PACKET;

   /* Build the DHCP HEADER  */
   (void) mqx_htonc(header_ptr->OP,    DHCPOP_BOOTREQUEST);
   (void) mqx_htonc(header_ptr->HTYPE, dhcp_ptr->HANDLE->DEV_TYPE);
   (void) mqx_htonc(header_ptr->HLEN,  dhcp_ptr->HANDLE->DEV_ADDRLEN);
   (void) mqx_htonc(header_ptr->HOPS,  0);
   /* Pick a random transaction ID */
   xid = RTCS_rand();
   (void) mqx_htonl(header_ptr->XID, xid);

   /* mqx_htons(header_ptr->FLAGS, DHCPFLAG_BROADCAST); */
   (void) mqx_htons(header_ptr->FLAGS, _DHCP_broadcast ? DHCPFLAG_BROADCAST : 0);
   
   if ( dhcp_ptr->STATE == DHCP_INFORMED ) {
      (void) mqx_htonl(header_ptr->CIADDR, dhcp_ptr->DATA.ClientIPaddress);
   } else {
      (void) mqx_htonl(header_ptr->CIADDR, INADDR_ANY);
   } /* Endif */
   (void) mqx_htonl(header_ptr->YIADDR, INADDR_ANY);
   (void) mqx_htonl(header_ptr->SIADDR, INADDR_ANY);
   (void) mqx_htonl(header_ptr->GIADDR, INADDR_ANY);
...
{ /* Body */
   ...
   unsigned char       addr_buffer[6];
   unsigned char  *buffer_end   = parms->OPT_BUFF
                              + parms->OPT_BUFF_SIZE - 1;

   outp = dhcp_ptr->PACKET + sizeof(DHCP_HEADER);
   (void) mqx_htonl(outp, DHCP_MAGIC);            outp += 4;
   (void) mqx_htonc(outp, DHCPOPT_MSGTYPE);       outp++;
   (void) mqx_htonc(outp, 1);                     outp++;
   dhcp_ptr->MSGTYPE_PTR = outp;
   if ( dhcp_ptr->STATE == DHCP_INFORMED ) {
      (void) mqx_htonc(outp, DHCPTYPE_DHCPINFORM);
   } else  {
      (void) mqx_htonc(outp, DHCPTYPE_DHCPDISCOVER);
   } /* Endif */
   outp++;
   /*
   ...
   */
   ...
   if (opt_ptr) {
      ...
      if ( mqx_ntohs(opt_ptr + 2) < DHCP_MIN_MESSAGE_SIZE ) {
         (void) mqx_htonc(outp, DHCPOPT_MSGSIZE);       outp++;
         (void) mqx_htonc(outp, 2);                     outp++;
         (void) mqx_htons(outp, DHCP_MIN_MESSAGE_SIZE); outp += 2;
      } else  {
         _mem_copy(opt_ptr, outp, 4);
         outp += 4;
      } /* Endif */
      for (i = 0; i < 4; i++) {
         (void) mqx_htonc(opt_ptr, 0);
         opt_ptr++;
      } /* Endfor */
   } /* Endif */
   ...
   */
   ...
   if ( opt_ptr ) {
      for (i = 0; i < 6; i++) {
         addr_buffer[i] = mqx_ntohc(opt_ptr);
         (void) mqx_htonc(opt_ptr, 0);
         opt_ptr++;
      } /* Endfor */
   }/* Endif */
   ...
   */
   ...
   while ( temp_ptr ) {
      temp_ptr = parms->OPT_BUFF;
      while ((mqx_ntohc(temp_ptr) == 0) && (temp_ptr < buffer_end)) {
         temp_ptr++;
      } /* Endwhile */
      if ( mqx_ntohc(temp_ptr) != 0 ) {
         new_total_length = buffer_end - temp_ptr + 1;
         len = mqx_ntohc(temp_ptr + 1) + 2;
   ...
   if (opt_ptr && (dhcp_ptr->STATE != DHCP_INFORMED)) {
      for ( i = 0; i < 6; i++ ) {
         (void) mqx_htonc(outp, addr_buffer[i]);
         outp++;
      } /* Endfor */
   } /* Endif */

   /* Add the DHCPEND flag last */
   (void) mqx_htonc(outp, DHCPOPT_END);
   outp++;
...
{ /* Body */
   DHCP_HEADER        *header_ptr = (DHCP_HEADER *)dhcp_ptr->PACKET;

   (void) mqx_htons(header_ptr->SECS, 0);
   /* Set initial timeout */
...
{  /* Body */
   ...
   for ( i = 0 ; i < 13 ; i++ ) {
         (void) mqx_htonc(outp, 0); outp++;
      } /* Endfor */
   outp -= 13;
   (void) mqx_htonc(dhcp_ptr->REQUEST_OPT_PTR, 0);
   ...
   if ( dhcp_ptr->STATE == DHCP_INITIALIZE ) {
      (void) mqx_htonc(dhcp_ptr->MSGTYPE_PTR, DHCPTYPE_DHCPDISCOVER);
      dhcp_ptr->CURRENT_PACKET_SIZE = dhcp_ptr->TOTAL_PACKET_SIZE;
   } else  {
      (void) mqx_htonc(dhcp_ptr->MSGTYPE_PTR, DHCPTYPE_DHCPREQUEST);
      if ( dhcp_ptr->STATE == DHCP_REQUESTING ||
           dhcp_ptr->STATE == DHCP_DECLINING  ||
           dhcp_ptr->STATE == DHCP_RELEASING ) {
         (void) mqx_htonc(outp, DHCPOPT_SERVERID); outp++;
         (void) mqx_htonc(outp, 4); outp++;
         if ( dhcp_ptr->STATE == DHCP_RELEASING ) {
            (void) mqx_htonc(dhcp_ptr->MSGTYPE_PTR, DHCPTYPE_DHCPRELEASE);
            (void) mqx_htonl(outp, dhcp_ptr->DATA.ServerIp); outp += 4;
         } else  {
            (void) mqx_htonl(outp, dhcp_ptr->NEW_DATA.ServerIp); outp += 4;
            (void) mqx_htonc(outp, DHCPOPT_ADDRESS); outp++;
            (void) mqx_htonc(outp, 4); outp++;
            (void) mqx_htonl(outp, dhcp_ptr->NEW_DATA.ClientIPaddress); outp += 4;
            if (dhcp_ptr->STATE == DHCP_DECLINING ) {
               (void) mqx_htonl(header_ptr->CIADDR, INADDR_ANY);
               (void) mqx_htonc(dhcp_ptr->MSGTYPE_PTR, DHCPTYPE_DHCPDECLINE);
            } else  {
      ...
      (void) mqx_htonc(outp, DHCPOPT_END); outp++;
      dhcp_ptr->CURRENT_PACKET_SIZE = outp - dhcp_ptr->PACKET;
   }/* Endif */
...
{ /* Body */
   DHCP_CLNT_STRUCT     *dhcp_ptr = event->PRIVATE;
   DHCP_HEADER          *header_ptr;
   RTCSPCB_PTR          pcb_ptr;
   ...
   header_ptr = (DHCP_HEADER *)dhcp_ptr->PACKET;
   (void) mqx_htons(header_ptr->SECS, dhcp_ptr->SECS);
...
static bool DHCPCLNT_rebind_lease
   (
   TCPIP_EVENT_PTR   event
...
static bool DHCPCLNT_renew_lease
   (
   TCPIP_EVENT_PTR   event
   /* [IN/OUT] the renew event */
   )
...
static bool DHCPCLNT_terminate_lease
   (
   TCPIP_EVENT_PTR   event
...
static bool DHCPCLNT_bind_attempt_timeout
   (
   TCPIP_EVENT_PTR   event
   /* [IN/OUT] the termination event */
   )
...
{  /* Body */
    uint32_t     old_state = dhcp_ptr->STATE;
...
void DHCPCLNT_release_internal
   (
      TCPIP_PARM_IF_DHCP_RELEASE  *parms
         /* [IN/OUT] This is the DHCP Client structure from the application */
   )
{  /* Body */
   ...
   if ( if_ptr->BOOT != NULL ) {
      dhcp_ptr = (DHCP_CLNT_STRUCT *)if_ptr->BOOT;
...
{ /* Body */
   ...
   header_ptr = (DHCP_HEADER *)RTCSPCB_DATA(pcb_ptr);

   /* Get our IP address, and pick the default netmask */
   dhcp_ptr->NEW_PARMS_BIND.address = mqx_ntohl(header_ptr->YIADDR);
   ...
   opt = (unsigned char *)header_ptr + sizeof(DHCP_HEADER) + 4;
...
#define DHCP_NEXTOPT    opt += optlen; \
   ...
   while (len) {
      ...
      case DHCPOPT_LEASE:
         ...
         dhcp_ptr->NEW_DATA.LEASE = mqx_ntohl(opt);
         opt += 4;
         break;

      case DHCPOPT_MASK:
         if (optlen != 4) {DHCP_NEXTOPT;}
         dhcp_ptr->NEW_PARMS_BIND.netmask = mqx_ntohl(opt);
      ...
      case DHCPOPT_RENEWALL:
         ...
         dhcp_ptr->NEW_RENEW_TIME = mqx_ntohl(opt);
         opt += DHCPSIZE_LEASE;
         break;
      ...
      case DHCPOPT_REBINDING:
         ...
         dhcp_ptr->NEW_REBIND_TIME = mqx_ntohl(opt);
         opt += DHCPSIZE_LEASE;
         break;
...
static unsigned char DHCPCLNT_verify_packet
 (
 DHCP_CLNT_STRUCT  *dhcp_ptr,
 RTCSPCB_PTR            pcb_ptr,
 unsigned char             *bootreply,
...
{ /* Body */
   DHCP_HEADER           *send_header_ptr;
   DHCP_HEADER           *header_ptr;
   unsigned char                 *opt;
   uint32_t                len;
   unsigned char                 *packet_option;
   ...
   send_header_ptr = (DHCP_HEADER *)dhcp_ptr->PACKET;
   header_ptr = (DHCP_HEADER *)bootreply;
   /* Make sure the XID matches */
   if (mqx_ntohl(header_ptr->XID) != mqx_ntohl(send_header_ptr->XID)) {
   ...
   *temp_addr = dhcp_ptr->NEW_DATA.ServerIp;
   ...
   dhcp_ptr->NEW_DATA.ClientIPaddress = mqx_ntohl(header_ptr->YIADDR);

   packet_option = DHCP_find_option(opt, len, DHCPOPT_SERVERID);
   if ( !packet_option || mqx_ntohc(packet_option + 1) != 4 ) {
      ...
      *error = RTCSERR_DHCPCLNT_ERROR_DECLINED;
      ...
      packet_option += 2;
      *current_ip = mqx_ntohl(packet_option);
   } /* Endif */

   packet_option = DHCP_find_option(opt, len, DHCPOPT_MSGTYPE);
   if ( !packet_option || mqx_ntohc(packet_option + 1) != 1 ) {
   ...
   packet_option += 2;
   *error = RTCS_OK;
   return(mqx_ntohc(packet_option));
} /* Endbody */
...
{  /* Body */
   uint32_t                random;
   uint32_t                renewall_range;
   uint32_t                renewall_time;
   uint32_t                terminate_time;
...
{  /* Body */
   ...
   dhcp_ptr->DATA.SADDR = mqx_ntohl(header_ptr->SIADDR);
   dhcp_ptr->NEW_DATA.ClientIPaddress = mqx_ntohl(header_ptr->YIADDR);
...
{ /* Body */
   ...
   DHCP_CLNT_STRUCT      *dhcp_ptr = (DHCP_CLNT_STRUCT *)if_ptr->BOOT;
   unsigned char                 *bootreply;
...
* Returned Values : bool, TRUE if the address is in use by another host,
*                   FALSE otherwise
* Comments        :
*
*END*-----------------------------------------------------------------*/
...
static bool DHCPCLNT_verify_address
   (
   DHCP_CLNT_STRUCT  *dhcp_ptr
   ...
   )
{
    ...
    return UDP_send_internal(ucb, &fromaddr, &toaddr, pcb_ptr, flags);
}
