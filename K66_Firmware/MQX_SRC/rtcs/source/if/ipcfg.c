...
/* hidden global data */
static unsigned char            ipcfg_empty_string[]                = "";
...
*
*END*-----------------------------------------------------------------*/
uint32_t ipcfg_del_interface (uint32_t dev_num)
{
...
* Comments        : Returns const char *to name of given state.
*
*END*-----------------------------------------------------------------*/

const char * ipcfg_get_state_string (IPCFG_STATE state)
{
    static const char *ipcfg_states[] =
    {
        "INIT",         //"init",
        "UNBOUND",      //"unbound",
        "BUSY",         //"busy",
    ...
    if (state < sizeof (ipcfg_states)/sizeof(ipcfg_states[0]))
    {
    ...
    )
{
    ...
    if (device < IPCFG_DEVICE_COUNT)
    {
        if (ipcfg_data[device].actual_state != IPCFG_STATE_INIT)
        {
            ...
            if( RTCS_if_get_dns_addr(ipcfg_data[device].ihandle, n, &dns_addr) == TRUE)
            {
                result = dns_addr;
            }
        }
    ...
    )
{
    ...
    if (device < IPCFG_DEVICE_COUNT)
    {
        if (ipcfg_data[device].actual_state != IPCFG_STATE_INIT)
        {
            if(RTCS_if_add_dns_addr(ipcfg_data[device].ihandle, address) == RTCS_OK)
            {
                result = TRUE;
            }
        }
    ...
    )
{
    ...
    if (device < IPCFG_DEVICE_COUNT)
    {
        if (ipcfg_data[device].actual_state != IPCFG_STATE_INIT)
        {
            if(RTCS_if_del_dns_addr(ipcfg_data[device].ihandle, address) == RTCS_OK)
            {
                result = TRUE;
            }
        }
...
unsigned char *ipcfg_get_tftp_servername
    (
        uint32_t device
    )
{
...
unsigned char *ipcfg_get_boot_filename
    (
        uint32_t device
    )
{
...
static uint32_t ipcfg_unbind_internal_actual
    (
        uint32_t device
    )
{
...
static uint32_t ipcfg_unbind_internal
    (
        uint32_t device
    )
{
...
static uint32_t ipcfg_bind_staticip_internal
    (
        uint32_t                     device,
...
static uint32_t ipcfg_bind_autoip_internal_actual
    (
        uint32_t                     device,
...
static uint32_t ipcfg_bind_autoip_internal
    (
        uint32_t                     device,
        IPCFG_IP_ADDRESS_DATA_PTR   auto_ip_data
    )
...
static int32_t ipcfg_dhcp_choice_func
    (
    ...
    )
{
    ...
    uint32_t             device = ipcfg_get_device_number (ihandle);
#if RTCSCFG_IPCFG_ENABLE_DNS
    IPIF_PARM           dns_parms;
#endif
    if (device >= IPCFG_DEVICE_COUNT)
    ...
    else
    {
        ...
        {
            optptr++;
            optlen = mqx_ntohc (optptr);
            optptr++;
            if (optlen >= 4)
            {
                ipcfg_data[device].actual_ip_data.mask = mqx_ntohl  (optptr);
            }
        }
        ...
        if (optptr) {
            ...
            if (optlen >= 4)
            {
                ipcfg_data[device].actual_ip_data.gateway = mqx_ntohl (optptr);
            }
        }
#endif
        ...
        if (optptr)
        {
            ...
            if (optlen >= 4)
            {
                ipcfg_data[device].dns = mqx_ntohl (optptr);
                dns_parms.ihandle = ipcfg_data[device].ihandle;
                dns_parms.address = ipcfg_data[device].dns;
                ip_if_add_dns_addr(&dns_parms);
            }
        }
#endif
...
static bool ipcfg_dhcp_nak_func
    (
        unsigned char *buffer_ptr,
        uint32_t buffer_size,
        _rtcs_if_handle ihandle
...
static bool ipcfg_dhcp_unbind_func
    (
        _rtcs_if_handle ihandle
    )
{
...
*
*END--------------------------------------------------------------------*/
static bool ipcfg_dhcp_fail_func
    (
        _rtcs_if_handle ihandle
    )
{
...
static uint32_t ipcfg_bind_dhcp_internal
    (
        uint32_t device,
        bool try_auto_ip
    )
{
    ...
    unsigned char           *optptr;
    unsigned char               DHCP_option_array[256];
    uint32_t             optlen = sizeof (DHCP_option_array);
    unsigned char               parm_options[] = {DHCPOPT_MASK, DHCPOPT_DNSSERVER, DHCPOPT_ROUTER, DHCPOPT_TFTPSERVER, DHCPOPT_BOOTFILENAME};
    uint32_t             error = 0;
...
static uint32_t ipcfg_poll_dhcp_internal
    (
        uint32_t                     device,
        bool                     try_auto_ip,
        IPCFG_IP_ADDRESS_DATA_PTR   auto_ip_data
...
{
#if RTCSCFG_IPCFG_ENABLE_DHCP && RTCSCFG_ENABLE_IP4
    uint32_t result = RTCSERR_IPCFG_BUSY;
...
static uint32_t ipcfg_bind_boot_internal
    (
        uint32_t device
    )
{
...
#endif
    uint32_t             result = RTCSERR_IPCFG_BIND;
    ...
    )
{
	...
	tec = _task_get_error();
    _mem_zero ((unsigned char *)&task_templ, sizeof (task_templ));
    ...
    task_templ.TASK_NAME          = (char *)ipcfg_task_name;
    task_templ.CREATION_PARAMETER = task_period_ms;

    tid = _task_create (0, 0, (uint32_t)&task_templ);
    ...
    )
{
    ...
    do
    {
        tid = _task_get_id_from_name ((char *)&ipcfg_task_name);
        if (tid != MQX_NULL_TASK_ID)
        {
...
*
*END--------------------------------------------------------------------*/
uint32_t ipcfg_release_device(uint32_t  device )
{
    _enet_handle    ehandle;
    _rtcs_if_handle ihandle;
    uint32_t         error = 0;
...
*
*END--------------------------------------------------------------------*/
uint32_t ipcfg_release_interface ( uint32_t dev_num)
{
    if (dev_num < IPCFG_DEVICE_COUNT)
    {
