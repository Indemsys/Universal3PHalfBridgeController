...
static uint16_t  pcm_wTsaBuffSize;        /* Dynamic TSA buffer size */
static uint8_t*   pcm_nTsaBuffAddr;        /* Dynamic TSA buffer address */
static uint16_t  pcm_nTsaTableIndex;

/* local function prototypes */
static int32_t FMSTR_TsaStrCmp(FMSTR_TSATBL_STRPTR p1, FMSTR_TSATBL_STRPTR p2);
#endif
...
{
    volatile uint8_t*  pResponse = pMessageIO;
...
static int32_t FMSTR_CheckMemSpace(uint8_t*  nAddrUser, uint8_t nSizeUser,
    uint8_t*  nAddrSafe, uint16_t wSizeSafe);

static int32_t FMSTR_CheckMemSpace(uint8_t*  nAddrUser, uint8_t nSizeUser,
                               uint8_t*  nAddrSafe, uint16_t wSizeSafe)
{
    int32_t bRet = FMSTR_FALSE;
...
int32_t FMSTR_CheckTsaSpace(uint8_t*  dwAddr, uint8_t nSize, int32_t bWriteAccess)
{
    ...
    for(nTableIndex=0U; (pte=FMSTR_TsaGetTable(nTableIndex, &cnt)) != NULL; nTableIndex++)
    {
        ...
        for(i=0U; i<cnt; i++)
        {
            ...
            if((nInfo & FMSTR_TSA_INFO_VAR_FLAG) && (!bWriteAccess || (nInfo & FMSTR_TSA_INFO_RWV_FLAG)))
            {
                ...
                if(sizeof(pte->addr.p) < sizeof(pte->addr.n))
                {
                    if(FMSTR_CheckMemSpace(dwAddr, nSize, (uint8_t* ) pte->addr.n, (uint16_t) (nInfo >> 2)))
                    {
...
#endif
                {
                    ...
                    if(FMSTR_CheckMemSpace(dwAddr, nSize, (uint8_t* ) pte->addr.p, (uint16_t) (nInfo >> 2)))
                    {
                        return FMSTR_TRUE; /* access granted! */
                    }
                }
