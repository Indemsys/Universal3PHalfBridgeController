...
typedef uint16_t FMSTR_TSA_TINDEX;
typedef uint16_t FMSTR_TSA_TSIZE;
...
{
    FMSTR_DUMMY dummy0;
    union { uint8_t*   p; uint8_t*  n; } name;
    FMSTR_DUMMY dummy1;
    union { uint8_t*   p; uint8_t*  n; } type;
    FMSTR_DUMMY dummy2;
    union { uint8_t*  p; uint8_t*  n; } addr;
    FMSTR_DUMMY dummy3;
    union { uint8_t*  p; uint8_t*  n; } info;
} FMSTR_TSA_ENTRY;
#else
typedef struct
{
    union { FMSTR_TSATBL_STRPTR  p; uint8_t*  n; } name;
    union { FMSTR_TSATBL_STRPTR  p; uint8_t*  n; } type;
    union { FMSTR_TSATBL_VOIDPTR p; uint8_t*  n; } addr;
    union { FMSTR_TSATBL_VOIDPTR p; uint8_t*  n; } info;
} FMSTR_TSA_ENTRY;
#endif
