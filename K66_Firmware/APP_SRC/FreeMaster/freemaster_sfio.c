...
static uint8_t pcm_nSfioRespLen;
...
uint8_t*  FMSTR_SfioFrame(uint8_t*  msg)
{
    uint8_t*  pResponse = msg;
    uint8_t i, nFrameLen, nByte;
    SFIO_U16 nRet = 0;

    /* get command and remember if it was even/odd */
    msg = FMSTR_ValueFromBuffer8(&nByte, msg);
    pcm_wSfioFlags.flg.bEvenRun = (nByte & 1) ? 0 : 1;
    pcm_wSfioFlags.flg.bLastOK = 0;

    /* get data length */
    msg = FMSTR_ValueFromBuffer8(&nFrameLen, msg);
    ...
    return FMSTR_CopyToBuffer(pResponse, (uint8_t* ) SFIO_GetOutputBuffer(), pcm_nSfioRespLen);
}
...
uint8_t*  FMSTR_SfioGetResp(uint8_t*  pMessageIO)
{
    ...
    return FMSTR_CopyToBuffer(pMessageIO, (uint8_t* ) SFIO_GetOutputBuffer(), pcm_nSfioRespLen);
}
