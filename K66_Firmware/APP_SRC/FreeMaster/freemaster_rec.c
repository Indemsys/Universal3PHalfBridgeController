...
uint16_t  pcm_wRecTotalSmps;        /* number of samples to measure */

#if FMSTR_REC_STATIC_POSTTRIG == 0
uint16_t  pcm_wRecPostTrigger;      /* number of post-trigger samples to keep */
#endif

#if (FMSTR_USE_FASTREC) == 0
uint8_t   pcm_nRecTriggerMode;      /* trigger mode (0 = disabled, 1 = _/, 2 = \_) */
#endif

#if (FMSTR_REC_STATIC_DIVISOR) == 0
uint16_t  pcm_wRecTimeDiv;          /* divisor of recorder "clock" */
#endif

uint8_t    pcm_nRecVarCount;        /* number of active recorder variables */
uint8_t*   pcm_pRecVarAddr[FMSTR_MAX_REC_VARS]; /* addresses of recorded variables */
uint8_t pcm_pRecVarSize[FMSTR_MAX_REC_VARS]; /* sizes of recorded variables */

/* runtime variables  */
#if (FMSTR_REC_STATIC_DIVISOR) != 1
uint16_t  pcm_wRecTimeDivCtr;       /* recorder "clock" divisor counter */
#endif

uint16_t  pcm_wStoprecCountDown;    /* post-trigger countdown counter */
...
#if (FMSTR_USE_FASTREC) == 0
uint16_t   pcm_wRecBuffStartIx;     /* first sample index */

/* Recorder buffer pointers */
uint8_t*  pcm_dwRecWritePtr;        /* write pointer in recorder buffer */
uint8_t*  pcm_dwRecEndBuffPtr;      /* pointer to end of active recorder buffer */

/* configuration variables */
static uint8_t*  pcm_nTrgVarAddr;          /* trigger variable address */
static uint8_t   pcm_nTrgVarSize;          /* trigger variable threshold size */
static uint8_t   pcm_bTrgVarSigned;        /* trigger compare mode (0 = unsigned, 1 = signed) */
...
static uint8_t*   pcm_nRecBuffAddr;        /* recorder buffer address */
#if FMSTR_REC_OWNBUFF
static uint32_t  pcm_wRecBuffSize;        /* recorder buffer size */
#endif
/* compare functions prototype */
typedef int32_t (*FMSTR_PCOMPAREFUNC)(void);
...
static uint8_t pcm_pOwnRecBuffer[FMSTR_REC_BUFF_SIZE];
...
static int32_t FMSTR_Compare8S(void);
static int32_t FMSTR_Compare8U(void);
static int32_t FMSTR_Compare16S(void);
static int32_t FMSTR_Compare16U(void);
static int32_t FMSTR_Compare32S(void);
static int32_t FMSTR_Compare32U(void);
#if FMSTR_REC_FLOAT_TRIG
static int32_t FMSTR_Comparefloat(void);
#endif
...
void FMSTR_SetUpRecBuff(uint8_t*  pBuffer, uint32_t nBuffSize)
{
#if FMSTR_REC_OWNBUFF
...
uint8_t*  FMSTR_SetUpRec(uint8_t*  pMessageIO)
{
    ...
    if((!pcm_nRecVarCount) || (pcm_nRecVarCount > (uint8_t)FMSTR_MAX_REC_VARS))
    {
    ...
    for(i=0U; i<pcm_nRecVarCount; i++)
    {
        ...
        if(!FMSTR_CheckTsaSpace(pcm_pRecVarAddr[i], (uint8_t)sz, 0U))
        {
#if FMSTR_REC_COMMON_ERR_CODES
            goto FMSTR_SetUpRec_exit_error;
#else
...
#if FMSTR_USE_TSA && FMSTR_USE_TSA_SAFETY
        if(!FMSTR_CheckTsaSpace(pcm_nTrgVarAddr, (uint8_t)pcm_nTrgVarSize, 0U))
        {
#if FMSTR_REC_COMMON_ERR_CODES
            goto FMSTR_SetUpRec_exit_error;
#else
    ...
    blen = (uint32_t) (pcm_wRecTotalSmps * nRecVarsetSize / FMSTR_CFG_BUS_WIDTH);
...
{
    uint16_t nResponseCode = (uint16_t) (pcm_wRecFlags.flg.bIsRunning ?
    ...
    /* get run/stop status */
    return FMSTR_ConstToBuffer8(pMessageIO, (uint8_t) nResponseCode);
}
...
uint32_t FMSTR_GetRecBuffSize()
{
#if FMSTR_REC_OWNBUFF
    return pcm_wRecBuffSize;
#else
    return (uint32_t) FMSTR_REC_BUFF_SIZE;
#endif
}
...
int32_t FMSTR_IsInRecBuffer(uint8_t*  dwAddr, uint8_t nSize)
{
    int32_t bRet = 0U;

    if(dwAddr >= pcm_nRecBuffAddr)
    {
        bRet = (int32_t)((dwAddr + nSize) <= (pcm_nRecBuffAddr + FMSTR_GetRecBuffSize()) ? FMSTR_TRUE : FMSTR_FALSE);
    }
...
#define CMP(v,t) ((int32_t)(((v) < (t)) ? 0 : 1))

#if FMSTR_CFG_BUS_WIDTH == 1U

static int32_t FMSTR_Compare8S()
{
    return CMP(FMSTR_GetS8(pcm_nTrgVarAddr), pcm_uTrgThreshold.s8);
}

static int32_t FMSTR_Compare8U()
{
...
void FMSTR_SetUpRecBuff(uint8_t*  pBuffer, uint16_t wBuffSize)
{
    FMSTR_UNUSED(pBuffer);
