...
static uint8_t  pcm_nScopeVarCount;        /* number of active scope variables */
static uint8_t*   pcm_pScopeVarAddr[FMSTR_MAX_SCOPE_VARS]; /* addresses of scope variables */
static uint8_t pcm_pScopeVarSize[FMSTR_MAX_SCOPE_VARS]; /* sizes of scope variables */
...
uint8_t*  FMSTR_SetUpScope(uint8_t*  pMessageIO)
{
    ...
    if(!nVarCnt || nVarCnt > (uint8_t)FMSTR_MAX_SCOPE_VARS)
    {
    ...
    for(i=0U; i<nVarCnt; i++)
    {
        ...
        if(!FMSTR_CheckTsaSpace(pcm_pScopeVarAddr[i], (uint8_t) sz, 0U))
        {
