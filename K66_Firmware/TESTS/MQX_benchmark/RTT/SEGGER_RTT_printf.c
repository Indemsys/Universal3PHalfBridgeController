...
int SEGGER_RTT_vprintf(unsigned BufferIndex, const char *sFormat, va_list *pParamList)
{
  ...
  unsigned FieldWidth;
  char rtt_printf_buf[SEGGER_RTT_PRINTF_BUFFER_SIZE];

  BufferDesc.pBuffer        = rtt_printf_buf;
  ...
  do
  {
    ...
    if (c == '%')
    {
      ...
      do
      {
        ...
        {
        case '-':
          FormatFlags |= FORMAT_FLAG_LEFT_JUSTIFY; sFormat++; break;
        case '0':
          FormatFlags |= FORMAT_FLAG_PAD_ZERO;     sFormat++; break;
        case '+':
          FormatFlags |= FORMAT_FLAG_PRINT_SIGN;   sFormat++; break;
        case '#':
          FormatFlags |= FORMAT_FLAG_ALTERNATE;    sFormat++; break;
        default:
          goto FilterFieldWidth;                   break;
        }
      }
      while (1);
      //
  ...
  if (BufferDesc.ReturnValue > 0)
  {
    ...
    if (BufferDesc.Cnt != 0)
    {
      SEGGER_RTT_Write(BufferIndex, rtt_printf_buf, BufferDesc.Cnt);
    }
...
/*------------------------------------------------------------------------------
 ...
 \return int
 ------------------------------------------------------------------------------*/
int RTT_terminal_printf(const char *sFormat, ...)
{
  va_list ParamList;
